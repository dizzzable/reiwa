...
в `.env` указывает на публичный домен панели, и обращения к ней идут через
обычный DNS.
...
при этом пусто: они до неё не дошли.

**Сначала почините резолверы на хосте кабинета**, а не в compose — в
...
Встроенный резолвер Docker при этом остаётся и продолжает разрешать имена
сервисов — одиночная установка с `REZEIS_HOST=rezeis` не ломается, — а чинится
DNS и к панели, и к `api.telegram.org`.

Если и это не помогает, а у панели статический адрес, его можно закрепить в
обход DNS оверлеем `docker-compose.pin-panel.yml`:

```bash
dig +short <ваш REZEIS_HOST>          # узнать адрес
echo 'REZEIS_HOST_IP=203.0.113.10' >> .env
docker compose -f docker-compose.yml -f docker-compose.pin-panel.yml up -d
```

⚠️ Прибитый адрес **не устаревает сам**. Если панель переедет или сменит IP,
кабинет будет молча стучаться в мёртвый адрес — хуже временного сбоя DNS, который
проходит сам. Заводите только при статическом IP и впишите смену адреса в порядок
переезда панели. Оверлей нужно указывать при **каждом** `up`.
