...
<p align="center">
  <a href="https://github.com/dizzzable/reiwa/releases/latest"><img src="https://img.shields.io/badge/version-0.9.7.51-blue" alt="Version" /></a>
...
- 🎨 **PWA-кабинет** на React 19 с операторским брендингом: 104 концептуальные темы, настраиваемые карточки и безопасные GPU/CSS-эффекты
...
docker pull ghcr.io/dizzzable/reiwa:v0.9.7.51
```

Доступные теги: `latest` (последний выпуск — двигается только push'ем тега `v*`), `v0.9.7.51` (тег релиза), `main` (текущая ветка `main`, ещё не выпуск), плюс `sha-<short>` для каждого собранного коммита. Прод-`docker-compose.yml` использует `latest`, то есть получает только выпуски.
...
### Если панель на другом VPS и DNS ненадёжен

Раздельная установка сама по себе ничего дополнительного не требует: `REZEIS_HOST`
в `.env` указывает на публичный домен панели, и обращения к ней идут через обычный
DNS.

Но если резолвер у провайдера подводит, `getaddrinfo EAI_AGAIN` кладёт **все**
бизнес-операции разом — запросы висят до таймаута и отвечают 503, а в логе панели
при этом пусто: они до неё не дошли. Оператору приходит карточка про отменённый
запрос, в которой про DNS не сказано ничего; настоящая причина видна только в логе
контейнера:

```
WARN: [api] Upstream temporarily unreachable
  err: getaddrinfo EAI_AGAIN <домен панели>
```

**Чинится на хосте кабинета, а не в compose** — резолверами демона Docker.

⚠️ `daemon.json` может уже существовать (драйвер логов, зеркала реестра,
storage-driver). **Не перезаписывайте его целиком** — потеряете эти настройки.
Сначала посмотрите, что там:

```bash
sudo cat /etc/docker/daemon.json 2>/dev/null || echo "файла нет"
```
...
**Если файл есть** — добавьте в него только ключ `dns`, сохранив остальное.
С резервной копией, потому что ошибка в JSON не даст демону подняться:

```bash
sudo cp /etc/docker/daemon.json /etc/docker/daemon.json.bak
sudo jq '. + {"dns": ["1.1.1.1", "8.8.8.8"]}' /etc/docker/daemon.json.bak | sudo tee /etc/docker/daemon.json
```

Нет `jq` — поставьте (`sudo apt install -y jq`) либо впишите ключ руками:
верхним уровнем объекта, рядом с существующими, не трогая их.

Перед рестартом убедитесь, что JSON цел — с битым файлом демон не запустится, и
поднимать его придётся уже из резервной копии:

```bash
python3 -m json.tool /etc/docker/daemon.json > /dev/null && echo "JSON валиден"
```
...
⚠️ Перезапуск демона **перезапускает контейнеры** — короткий простой кабинета.
Делайте в спокойное время. (`"live-restore": true` в том же файле его избегает, но
на смену настроек DNS всё равно нужен рестарт.)

**Проверка, что помогло** — резолвим домен панели изнутри самого контейнера,
то есть ровно так, как это делает приложение:

```bash
docker exec reiwa node -e "require('dns').resolve4(process.env.REZEIS_HOST, (e, a) => console.log(e ? 'ОШИБКА: ' + e.code : 'ОК: ' + a))"
```

Должно напечатать `ОК:` и адрес. `ОШИБКА: EAI_AGAIN` означает, что резолверы всё
ещё не отвечают.

**Почему именно демон, а не `dns:` в compose.** Ключ `dns:` у сервиса **замещает**
встроенный резолвер Docker (127.0.0.11), а это он превращает `REZEIS_HOST=rezeis`
в адрес контейнера. На одиночном VPS панель после такого перестанет находиться
вообще. Настройка демона так не ломает: встроенный резолвер остаётся на месте и
лишь пересылает внешние имена новым адресам — поэтому чинятся сразу и панель, и
`api.telegram.org`, и оба сервиса кабинета (`reiwa` и `reiwa-bot`, который ходит к
панели сам).
...
- Определение локали, operator-managed конфиг-кэш, баннеры и динамические экраны из Bot Studio
- `/start payment_return` — возврат после оплаты из Mini App
- Команды: меню, подписки, покупка, промокод, рефералы, помощь и переходы в Mini App
...
- **Dashboard** — карусель карточек подписок с брендингом оператора: градиент, паттерн, логотип, геометрия, стекло, текст и анимированный фон; полоса трафика, срок и устройства
- **Покупка** — мастер: тариф → срок → платёжная система → оплата
...
### 🙋 Самообслуживание и коммуникация

- **Дополнительные услуги** — покупка и управление расширениями тарифа, включая трафик и устройства
- **Уведомления** — лента событий, настройки категорий, переход к связанному действию и отметка всех сообщений прочитанными
- **Поддержка** — обращения из кабинета и гостевой вход в поддержку до авторизации; FAQ поддерживает операторские медиа-материалы
- **AI-помощник** — пользовательская точка входа в поддержку, когда оператор включает её в Rezeis
- **Программа роста** — реферальные ссылки, партнёрский статус, баллы, промокоды, задания и рекламные предложения, если они активированы оператором
- **Настройки кабинета** — язык, PWA-установка, уведомления и разрешённая оператором светлая/тёмная вариация бренда

### 🔑 Вход и связка аккаунтов

- **Telegram Mini App:** подписанный `initData` автоматически создаёт или восстанавливает web-сессию; пользователь, пришедший из бота, не должен видеть форму логина
- **Обычный браузер:** вход по логину/паролю, существующей cookie-сессии или одноразовой ссылке возврата
- **Единый аккаунт:** Telegram-first пользователь может создать web-учётную запись или привязать существующую, чтобы один кабинет работал и в Telegram, и в браузере
- **Внешняя авторизация:** на странице входа отображаются только включённые оператором способы OAuth
...
### 🎨 Контракт брендинга WEB Reiwa

Внешний вид задаёт оператор в **Rezeis → WEB Reiwa**; Reiwa получает уже разрешённые токены брендинга через публичный bootstrap-конфиг и не зависит от каталога админ-панели во время отрисовки. При кратковременной ошибке обновления браузер сохраняет последнюю валидную конфигурацию, поэтому кабинет не должен мигать стандартной темой.

- **Пресеты:** восемь исторических тем и каталог из **104 концептуальных пресетов**. Концепт передаёт палитру, фон приложения, типографику, поверхности, радиусы, градиент/паттерн и параметры карточки.
- **Светлый/тёмный режим:** оператор выбирает сам концепт и режим по умолчанию. При политике `user-selectable` пользователь может сменить только светлую/тёмную вариацию этого же концепта — выбрать другой операторский пресет он не может.
- **Карточки подписок:** глобальные параметры являются базой для всех карточек. Для позиций можно создать до 20 слотов: новый слот наследует глобальный эффект, а отдельный режим `override` осознанно меняет эффект и его параметры только для этой позиции. Статический градиент слота независим от режима эффекта.
- **Читаемость и композиция:** режим текста (`auto`, светлый, тёмный или свой цвет) и стеклянный слой настраиваются отдельно от эффекта. Изменение контраста не должно подменять градиент или палитру, выбранные оператором.
- **Безопасный рендер:** градиент и паттерн всегда являются базовым слоем. Если браузер или GPU не поддерживает выбранный Canvas/WebGL-эффект, Reiwa показывает CSS-версию **того же** эффекта с операторской палитрой и прозрачностью, а не подменяет его чужим эффектом или пустой карточкой.
