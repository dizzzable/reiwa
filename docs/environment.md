...
работающем Redis, **потом** новый файл:

```bash
docker exec reiwa-redis valkey-cli -a "$REDIS_PASSWORD" CONFIG SET appendonly yes
# подождать несколько секунд: в INFO persistence должно быть aof_rewrite_in_progress:0
docker exec reiwa-redis valkey-cli -a "$REDIS_PASSWORD" INFO persistence | grep aof_rewrite_in_progress
# заменить docker-compose.yml и перезапустить
docker compose up -d
```
...
- **Последние рабочие копии.** Каждая группа, которую панель отдала, лежит в
  ...
  Без TTL: копию заменяет следующий ответ панели.
