...
export const RU_PACK: Readonly<Record<string, string>> = {
  ...
  // Четыре кнопки, которые ДЕЙСТВИТЕЛЬНО рисуются главной клавиатурой. Их не
  // было в паке вовсе: подпись бралась из настроек оператора, а та одна на оба
  // языка — англоязычный клиент видел «Мой кабинет».
  'menu.btn_webapp': 'Открыть приложение',
  'menu.btn_cabinet': 'Мой кабинет',
  'menu.btn_invite': 'Пригласить',
  'menu.btn_rules': 'Правила',
  'menu.btn_help': 'Помощь',

  // ── AI support ──────────────────────────────────────────────────────────────
  // Эта страница была единственной в боте без перевода вовсе: англоязычный
  // клиент получал английский ответ модели, обёрнутый в русский экран, русскую
  // кнопку и русский отказ. Обращение на «вы», как во всём остальном паке.
  'ai_support.exit_button': '❌ Выйти из поддержки',
  'ai_support.unavailable':
    '😔 *AI-поддержка временно недоступна*\n\nПожалуйста, обратитесь к оператору через /help',
  'ai_support.intro':
    '🤖 *Режим AI-поддержки*\n\nЗадайте любой вопрос о сервисе — расскажу о тарифах, помогу с настройкой приложения, подскажу решение проблемы.\n\nПросто напишите вопрос, и я отвечу 📝\n\n_Чтобы выйти, напишите /cancel_',
  'ai_support.exited':
    '✅ *Режим AI-поддержки завершён*\n\nЕсли понадобится помощь — напишите /support или /help',
  'ai_support.rate_limited': '⏳ Слишком много сообщений подряд. Подождите немного и попробуйте снова.',
  'ai_support.failed': '😔 Не удалось получить ответ. Попробуйте ещё раз или напишите /cancel для выхода.',
  ...
  // Без названия сервиса: это название панели, а не бренд оператора. Своё имя
  // оператор вписывает в «Карте бота» (экран приглашения).
  'invite.share_prompt': 'Привет! Попробуй — быстрый и надёжный VPN.',
  ...
  // Подпись кнопки рядом с полем ввода, когда оператор выбрал «открывать
  // кабинет». Telegram показывает ОДНУ подпись всем — у setChatMenuButton нет
  // кода языка, — поэтому при не русскоязычной аудитории её задают вручную.
  'menu_button.cabinet': 'Кабинет',
  // Telegram Stars. Первые три показывает сам Telegram при отказе, поэтому
  // они называют ситуацию, а не внутреннее состояние.
  'payments.stars.unknown_invoice': 'Счёт не найден. Оформите оплату заново.',
  ...
  'payments.stars.unavailable':
    'Не удалось подтвердить оплату. Звёзды не списаны — попробуйте ещё раз.',
  'payments.stars.received': '⭐️ Оплата получена. Подписка активируется.',
  'payments.stars.received_delayed':
    '⭐️ Оплата получена, но активация задерживается. Мы уже знаем о проблеме — напишите в поддержку, если через несколько минут ничего не изменится.',
  // `/paysupport` — Telegram ждёт этой команды от бота, принимающего оплату.
  // Копия своя, а не общая с `/help`: у человека, набравшего её, вопрос
  // конкретнее, и предзаполнение сообщения сразу называет тему.
  'paysupport.body':
    '💳 Вопросы по оплате\n\nОплату и возвраты разбирает поддержка вручную. Напишите нам — укажите дату платежа и способ оплаты, так ответим быстрее.',
  'paysupport.unavailable':
    '💳 Вопросы по оплате\n\nКонтакт поддержки пока не настроен. Напишите через кабинет — там же видна история платежей.',
  'paysupport.prefill': 'Здравствуйте! У меня вопрос по оплате.',
  // Сброс пароля от кабинета по ссылке `t.me/<бот>?start=pwreset` с экрана
  // «Забыли пароль?». Логин называется в сообщении — клиент мог забыть и его.
  'password_reset.link':
    '🔑 Сброс пароля\n\nЛогин вашего кабинета: {{login}}\n\nНажмите кнопку ниже и придумайте новый пароль. Ссылка сработает один раз и действует 15 минут.\n\nЕсли вы не запрашивали сброс — просто не нажимайте кнопку, пароль останется прежним.',
  'password_reset.button': '🔑 Задать новый пароль',
  'password_reset.no_account':
    'К этому Telegram не привязан логин от кабинета, поэтому сбрасывать нечего. Откройте кабинет из главного меню бота (/start) — через бота вход работает без пароля.',
  'password_reset.recently_sent':
    'Ссылку для сброса отправили меньше минуты назад — посмотрите сообщения выше или почту. Новую можно запросить через минуту.',
  'password_reset.hourly_limit':
    'За последний час уже отправлено пять ссылок для сброса — больше нельзя. Попробуйте позже.',
  'password_reset.unavailable':
    'Сейчас не получается отправить ссылку для сброса. Попробуйте позже или напишите в поддержку.',
