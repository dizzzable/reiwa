 ...
 * InvitedUsersList
 * ────────────────
 * Glass block listing the users this user has invited, newest first.
 ...
 *
 * Each row shows the invited user's display label (login → @username → name →
 * masked tg/email) and a "qualified" badge once they completed a purchase.
 */

import { useQuery, keepPreviousData } from "@tanstack/react-query";
import { motion } from "motion/react";
import { ChevronLeft, ChevronRight, CircleCheck, UserPlus, Users } from "lucide-react";
import { useState } from "react";
import { useTranslation } from "react-i18next";

import { getInvitedUsers } from "@/lib/api-client";
...
export function InvitedUsersList() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ["referrals", "invited", page],
    queryFn: () => getInvitedUsers(page, PAGE_SIZE),
    staleTime: 30_000,
    ...
    <div className="rounded-2xl border border-white/6 bg-white/3 p-4">
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-white">{t("referrals.invitedList")}</p>
        </div>
        {total > 0 && (
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-2 py-8 text-center">
          ...
          <p className="text-xs text-zinc-500">{t("referrals.invitedEmpty")}</p>
        </div>
      ) : (
        <div
        ...
        >
          {items.map((u, i) => (
            ...
            >
              ...
              {u.qualified ? (
                <span className="flex shrink-0 items-center gap-1 rounded-full bg-emerald-500/12 px-2 py-0.5 text-[10px] font-medium text-emerald-400">
                  <CircleCheck className="h-3 w-3" />
                  {t("referrals.qualified")}
                </span>
              ) : (
                <span className="shrink-0 rounded-full bg-white/6 px-2 py-0.5 text-[10px] font-medium text-zinc-500">
                  {t("referrals.pending")}
                </span>
              )}
            </motion.div>
