import type { RuDict } from './ru';

export const en: RuDict = {
  errorBoundary: {
    title: 'Something went wrong',
    body: 'An unexpected error occurred. We have received a report about it. Please try reloading the page.',
    reload: 'Reload',
  },
  guestSupport: {
    title: 'Support',
    subtitle: 'Describe your problem — no account needed. An operator will reply here.',
    entry: 'Need help? Contact support',
    form: {
      subject: 'Subject',
      subjectPlaceholder: 'Briefly: what do you need help with',
      message: 'Message',
      messagePlaceholder: 'Describe the problem in detail',
      email: 'Email (optional)',
      emailPlaceholder: 'you@example.com',
      emailHint: 'Add it to get a reply if you close the tab.',
      submit: 'Send request',
      sending: 'Sending…',
    },
    resume: {
      title: 'Save your resume code',
      body: 'Use this code to return to the conversation on another device. Keep it safe.',
      copy: 'Copy',
      copied: 'Copied',
      restoreLabel: 'Have a resume code?',
      restorePlaceholder: 'Paste the code',
      restoreButton: 'Open',
    },
    // The «Open conversation» button in a support reply email.
    link: {
      opening: 'Opening the conversation…',
      staleContinue: 'This link no longer opens the conversation. Below is the conversation open on this device.',
      staleNone:
        'This link no longer opens the conversation: either support has since sent a newer email, or access to the request has ended. Try “Open conversation” in the newest email (“Открыть переписку” if the email is in Russian) or your resume code; if that does not work, write to us again.',
      failed: 'The conversation could not be opened from the link.',
      failedCode: 'The conversation could not be opened with the code.',
      retry: 'Try again',
      confirmTitle: 'Open a different request?',
      confirmBody:
        'This device has the request “{{current}}” open. If you open “{{opening}}”, “{{current}}” stays open, but this device will stop showing it.',
      // Only ways back that exist: the code field is under the conversation too,
      // and letters reach only guests who left an email, when the operator has
      // email turned on.
      confirmWayBack:
        'You can return to “{{current}}” with its resume code, if you saved it: the “Have a resume code?” field will be under the conversation. Or with “Open conversation” (“Открыть переписку” in a Russian email) in a support email about it, if such emails reach you.',
      confirmOpen: 'Open the other request',
      confirmStay: 'Stay in the current one',
    },
    chat: {
      you: 'You',
      operator: 'Support',
      system: 'System',
      replyPlaceholder: 'Your message…',
      send: 'Send',
      close: 'Close ticket',
      closed: 'Ticket closed',
      closedNote: 'This conversation is finished and archived. To continue, start a new ticket.',
      newTicket: 'New ticket',
      empty: 'No messages yet.',
    },
    status: {
      open: 'Open',
      waiting_reply: 'Awaiting reply',
      closed: 'Closed',
    },
    errors: {
      required: 'Fill in subject and message',
      captcha: 'Verification failed. Please try again.',
      tooLong: 'Text is too long',
      rateLimited: 'Too many requests. Try again later.',
      notFound: 'Conversation not found or closed.',
      generic: 'Something went wrong. Please try again.',
    },
    disabled: {
      title: 'Support is temporarily unavailable',
      body: 'The anonymous chat is currently turned off. Please try again later.',
    },
  },
  accessMode: {
    banner: {
      INVITED: {
        title: 'Invite only',
        body: 'Registration is currently available by invite link only (from a friend or partner). If you already have an account, just sign in.',
      },
      PURCHASE_BLOCKED: {
        title: 'Purchases temporarily disabled',
        body: 'New purchases are currently unavailable — this feature is under maintenance. Existing subscriptions can be renewed as usual.',
      },
      REG_BLOCKED: {
        title: 'Registration disabled',
        body: 'New account registration is temporarily unavailable. You cannot start using the service right now.',
      },
      RESTRICTED: {
        title: 'Service temporarily unavailable',
        body: 'Maintenance is in progress. Existing VPN connections keep working; purchases, renewals and changes are temporarily unavailable.',
      },
    },
    cta: {
      disabled: 'Action unavailable',
    },
    invite: {
      codeLabel: 'Invite code',
      codePlaceholder: 'Paste your invite code',
      codeRequired: 'Enter an invite code',
      codeInvalid: 'Invite code is invalid or expired',
    },
    pendingNote: 'The panel is unavailable right now — trying again…',
  },
  common: {
    close: 'Close',
    gb: 'GB',
    primaryNav: 'Primary navigation',
    loadingPage: 'Loading page',
    swErrorTitle: 'Configuration Error',
    loading: 'Loading...',
    save: 'Save',
    cancel: 'Cancel',
    back: 'Back',
    next: 'Next',
    confirm: 'Confirm',
    retry: 'Retry',
    copy: 'Copy',
    copied: 'Copied',
    language: 'Language',
    languageRu: 'Русский',
    languageEn: 'English',
    error: 'Error',
    errorGeneric: 'Something went wrong',
    noData: 'No data',
  },
  bootstrap: {
    rateLimitedTitle: 'Too many attempts',
    rateLimited: 'Try again in {{seconds}} sec.',
    title: 'Initializing',
    description: 'Checking session...',
    notInTelegram: 'This app must be opened in Telegram',
    authFailed: 'Authentication failed',
    retry: 'Try again',
    openInTelegram: 'Open the app via Telegram to sign in.',
    loginError: 'Sign-in error',
    loginErrorFallback: 'Sign-in error',
    loginSuccess: 'Signed in',
    initializing: 'Initializing…',
    signingIn: 'Signing in…',
    connectingViaTelegram: 'Signing in via Telegram…',
    connecting: 'Connecting…',
    tagline: 'VPN Service',
    // Product gates (AccessMode) — safe for every user.
    registrationDisabled: 'Registration is currently disabled. Contact support.',
    inviteRequired: 'Invite-only registration. Open the bot via a referral link.',
    serviceRestricted: 'Service is temporarily unavailable. Try again later.',
    // Generic refusal — no operator/env diagnostics (those only arrive as
    // `debug` for BOT_DEV_ID from the server).
    accessDenied: 'Could not sign in. Try again later or contact support.',
  },
  /**
   * The mandatory-channel screen in the Telegram Mini App. The button labels
   * and the refusal were copied from the bot's English pack (`channel.*` in
   * `src/infrastructure/i18n/packs/en.pack.ts`) and are not linked to it: panel
   * edits to the bot's texts never reach this screen, `locales/en.override.json`
   * does (`locale-overlay.ts`). `body` names "I subscribed" and has to keep
   * matching `check`.
   */
  channelGate: {
    title: 'Subscribe to the channel',
    body: 'To use the service, subscribe to our channel, then tap "I subscribed".',
    join: '📢 Open channel',
    check: '✅ I subscribed',
    checking: 'Checking your subscription…',
    notSubscribed: '❌ You are not subscribed yet. Subscribe and try again.',
    checkFailed: 'Could not verify your subscription. Please try again in a moment.',
    rateLimited: 'Too many checks in a row. Try again in {{seconds}} sec.',
  },
  /**
   * Live events, in the customer’s own language.
   *
   * The panel sends one neutral sentence per event; these replace it. Keyed by
   * the event TYPE, so the dotted names are deliberate — see
   * `realtime-event-copy.ts`.
   */
  realtimeEvents: {
    'subscription.created': 'Your subscription is ready',
    'subscription.renewed': 'Your subscription has been extended',
    'subscription.expired': 'Your subscription has ended',
    'subscription.deleted': 'Your subscription has been removed',
    'subscription.upgraded': 'Your plan has changed',
    'user_hwid_revoked': 'A device has been unlinked from your subscription',
    'payment.completed': 'Payment received',
    'payment.failed': 'The payment did not go through',
    'promocode.activated': 'Your promo code has been applied',
    'referral.qualified': 'Your referral has been confirmed',
    'referral.reward_issued': 'A referral reward has been credited',
    'user.deleted': 'Your account has been deleted',
    'subscription.trial_granted': 'Your trial is active',
  },
  hints: {
    later: 'Maybe later',
  },
  servers: {
    title: 'Servers',
    subtitle_one: '{{count}} server available',
    subtitle_few: '{{count}} servers available',
    subtitle_many: '{{count}} servers available',
    subtitle_other: '{{count}} servers available',
    listHeading: 'Available servers',
    loading: 'Checking what is available…',
    empty: 'No servers for this subscription yet',
    recommendedLabel: 'Least busy',
    // "Fewest connections", never "fastest": nothing here measures the route
    // from this customer to that server, so speed would be a false claim.
    recommendedWhy: 'fewest connections right now',
    status: {
      online: 'Working',
      connecting: 'Connecting',
      offline: 'Not responding',
      unknown: 'No data',
    },
    // "In service for", not "N% available": this is how long the process has
    // been running. A node restarted five minutes ago is perfectly healthy and
    // reads five minutes.
    //
    // The `_few` and `_many` forms exist because `en` is typed as `typeof ru`
    // and the shape has to match; English never selects them.
    uptimeDays_one: 'in service for {{count}} day',
    uptimeDays_few: 'in service for {{count}} days',
    uptimeDays_many: 'in service for {{count}} days',
    uptimeDays_other: 'in service for {{count}} days',
    uptimeHours_one: 'in service for {{count}} hour',
    uptimeHours_few: 'in service for {{count}} hours',
    uptimeHours_many: 'in service for {{count}} hours',
    uptimeHours_other: 'in service for {{count}} hours',
    uptimeMinutes_one: 'in service for {{count}} minute',
    uptimeMinutes_few: 'in service for {{count}} minutes',
    uptimeMinutes_many: 'in service for {{count}} minutes',
    uptimeMinutes_other: 'in service for {{count}} minutes',
  },
  dashboard: {
    title: 'Dashboard',
    welcome: 'Welcome, {{name}}!',
    subscription: 'Subscription',
    subscriptionActive: 'Active',
    subscriptionExpired: 'Expired',
    subscriptionNone: 'No subscription',
    expires: 'Expires: {{date}}',
    buyNow: 'Buy subscription',
    renew: 'Renew',
    manage: 'Manage',
    notifications: 'Notifications',
    unread_one: '{{count}} unread',
    unread_few: '{{count}} unread',
    unread_many: '{{count}} unread',
    unread_other: '{{count}} unread',
  },
  connectHelp: {
    bannerTitle: "Couldn't connect?",
    bannerBody:
      "Your subscription is active, but the VPN hasn't connected yet. Tap “Connect” — we'll suggest an app for your device and add your subscription in one tap.",
    connect: 'Connect',
    support: 'Contact support',
    dismiss: 'Hide',
    tapConnect: 'Tap “Connect” to open the connection page.',
    settingsGroup: 'Connection',
    settingsSwitch: 'Connection help',
    settingsHint: "We'll remind you if the VPN never connects after a purchase or the start of a trial.",
  },
  trialCta: {
    titleFree: 'Try it free',
    subtitleFree: 'Activate a free trial and explore the service right away.',
    buttonFree: 'Try for free',
    titlePaid: 'Try the service',
    subtitlePaid: 'Start with a trial plan.',
    buttonPaid: 'Try for {{price}}',
    activating: 'Activating…',
    activationSteps: {
      '0': 'Activating your trial…',
      '1': 'Creating your profile…',
      '2': 'Setting up your connection…',
      '3': 'Almost done, just a moment…',
    },
    dismiss: 'Maybe later',
    errorGeneric: 'Could not activate the trial. Please try again.',
    errorAlreadyUsed: 'The trial has already been used.',
    errorHasSubscription: 'You already have an active subscription.',
    errorNotConfigured: 'The trial is unavailable right now.',
    errorRequiresPayment: 'The trial plan is only available by purchase.',
    errorInvitedOnly: 'The trial is available by invitation only.',
    titleLinkTelegram: 'Link Telegram',
    subtitleLinkTelegram: 'The trial unlocks once you link your Telegram account.',
    buttonLinkTelegram: 'Link Telegram',
  },
  subscription: {
    title: 'My subscription',
    plan: 'Plan',
    status: 'Status',
    expires: 'Expires',
    traffic: 'Traffic',
    devices: 'Devices',
    unlimited: 'Unlimited',
    renewAction: 'Renew',
    upgradeAction: 'Change plan',
    openVpn: 'Open VPN',
    copyLink: 'Copy link',
    noSubscription: 'You have no active subscription',
    buyFirst: 'Choose a plan to start using VPN',
    pageTitle: 'Subscription',
    previousCard: 'Previous subscription',
    nextCard: 'Next subscription',
    carouselItemAria: 'Subscription {{index}}',
    planFallback: 'Subscription',
    trialPeriod: 'Trial period',
    today: 'Today',
    daysLeftShort: '{{count}} d.',
    devicesLabel: 'Devices',
    typeLabel: 'Type',
    linkCopied: 'Link copied',
    noSubTip: 'You have no active subscription. Buy a plan to get started.',
    choosePlan: 'Choose a plan',
    expiresInWarning_one: 'Subscription expires in {{count}} day. Renew now.',
    expiresInWarning_few: 'Subscription expires in {{count}} days. Renew now.',
    expiresInWarning_many: 'Subscription expires in {{count}} days. Renew now.',
    expiresInWarning_other: 'Subscription expires in {{count}} days. Renew now.',
    renewFull: 'Renew subscription',
    buyNew: 'Buy a new subscription',
    buyForTrial: 'Buy a subscription',
    upgradePlan: 'Upgrade plan',
    manageDevices: 'Manage devices',
    limitReached:
      'Subscription limit reached. Delete one of your existing subscriptions or contact support.',
    limitReachedDetail:
      'Subscription limit reached ({{current}} of {{max}}). Delete one of your existing subscriptions or contact support if you need a higher limit.',
  },
  plans: {
    title: 'Plans',
    subtitle: 'Choose a plan that suits you',
    loading: 'Loading plans...',
    empty: 'Plans are temporarily unavailable',
    select: 'Select',
    traffic: 'Traffic',
    devices: 'Devices',
    popular: 'Popular',
    trafficPerMonth: '{{value}} GB/mo',
    devicesLimit: 'up to {{count}}',
    emptyAvailable: 'No plans available',
    trialConversion:
      'You have a trial subscription. The plan you choose will replace it: your connection link stays the same, and the term starts when you pay.',
    trialBadge: 'Trial',
    unlimited: 'Unlimited',
    devicesSuffix_one: '{{count}} device',
    devicesSuffix_few: '{{count}} devices',
    devicesSuffix_many: '{{count}} devices',
    devicesSuffix_other: '{{count}} devices',
    durationOptions_one: '{{count}} duration option',
    durationOptions_few: '{{count}} duration options',
    durationOptions_many: '{{count}} duration options',
    durationOptions_other: '{{count}} duration options',
    from: 'from',
    daysShort: 'd.',
    fromDuration_one: 'from {{count}} day',
    fromDuration_few: 'from {{count}} days',
    fromDuration_many: 'from {{count}} days',
    fromDuration_other: 'from {{count}} days',
    cashbackUpTo_one: 'up to +{{count}} point',
    cashbackUpTo_few: 'up to +{{count}} points',
    cashbackUpTo_many: 'up to +{{count}} points',
    cashbackUpTo_other: 'up to +{{count}} points',
  },
  purchase: {
    label: 'Purchase',
    back: '← Back',
    duration: {
      title: 'Choose duration',
      days_one: '{{count}} day',
      days_few: '{{count}} days',
      days_many: '{{count}} days',
      days_other: '{{count}} days',
      months_one: '{{count}} mo.',
      months_few: '{{count}} mo.',
      months_many: '{{count}} mo.',
      months_other: '{{count}} mo.',
      year: '1 year',
      cashback_one: '+{{count}} point',
      cashback_few: '+{{count}} points',
      cashback_many: '+{{count}} points',
      cashback_other: '+{{count}} points',
    },
    gateway: {
      title: 'Payment method',
      empty: 'Payment methods are temporarily unavailable',
      savedTitle: 'Saved cards',
      autopayCaption: 'for automatic charging',
      useSaved: 'Charge saved method',
    },
    quote: {
      title: 'Confirm payment',
      plan: 'Plan',
      duration: 'Duration',
      method: 'Method',
      savedMethod: 'Card',
      discount: 'Discount',
      total: 'Total',
      pay: 'Proceed to payment',
      change: 'Change',
      priceError: 'Failed to get the price. Try a different payment method.',
      payWithBalance: 'Pay with balance ({{amount}} {{currency}})',
      balancePaid: 'Paid from partner balance',
      balanceError: 'Failed to pay with balance',
      autopayTitle: 'Automatic charging',
      autopayHint:
        'The payment method is saved, and when your subscription is about to end the renewal is charged to it automatically. You can turn this off at any time under Payment methods.',
      autopayProviderHint:
        'Your bank sets up a subscription over SBP: this amount is charged now and then at the start of each term. You can turn it off at any time under Payment methods.',
      trialConversion:
        'Your trial subscription will become the «{{plan}}» plan. Your connection link stays the same, and the term starts when you pay.',
    },
    checkout: {
      creating: 'Creating payment…',
      trialConversionRequired:
        'You have a trial subscription — this purchase will move it onto the chosen plan. Check the amount and pay again.',
      autopayNotAvailable: 'Automatic charging is not available for this purchase. Choose the ordinary payment.',
      // The panel's PENDING_SIGN_UP; see the Russian dictionary.
      autopaySignUpPending:
        'An automatic payment for this purchase is already under way. Confirm it in your banking app, or wait up to {{minutes}} minutes for the request to expire and start again.',
      error: 'Failed to create payment. Try again later.',
      planUnavailable: 'This plan is no longer available. Please choose one from the updated list.',
      notAccepted: "This purchase can't be completed right now. If it keeps happening, contact support.",
    },
    noGateways: 'Payment methods are temporarily unavailable',
  },
  payment: {
    title: 'Payment',
    processing: 'Processing payment...',
    success: 'Payment completed',
    pending: 'Payment pending',
    failed: 'Payment failed',
    backToDashboard: 'Back to dashboard',
    viewSubscription: 'My subscription',
    tryAgain: 'Try again',
    // Abandoning an unpaid checkout. Never call this a refund — no money has
    // moved, and refunds in this product are handled by support only.
    abandonCheckout: 'Cancel this attempt',
    abandonDone: 'Attempt cancelled',
    abandonAtProvider: 'This payment already exists at the provider — finish it or let it expire.',
    abandonFailed: 'Could not cancel the attempt. Try again.',
  },
  activity: {
    title: 'Activity',
    tabTransactions: 'Transactions',
    tabNotifications: 'Notifications',
    noTransactions: 'No transactions yet',
    noNotifications: 'No notifications',
    markAllRead: 'Mark all read',
    tabTransactionsLabel: 'Transactions',
    emptyNotifications: 'No notifications yet',
    emptyTransactions: 'No transactions yet',
    status: {
      PENDING: 'Pending',
      COMPLETED: 'Completed',
      CANCELED: 'Canceled',
      FAILED: 'Failed',
      REFUNDED: 'Refunded',
    },
    txStatus: {
      COMPLETED: 'Paid',
      PENDING: 'Pending',
      FAILED: 'Failed',
      CANCELED: 'Canceled',
      REFUNDED: 'Refunded',
    },
    paymentFallback: 'Payment',
  },
  promo: {
    title: 'Promo code',
    description: 'Enter a promo code to get a discount or bonus',
    placeholder: 'PROMO123',
    activate: 'Activate',
    activating: 'Activating...',
    success: 'Promo code activated!',
    error: 'Invalid or expired promo code',
    history: 'Activation history',
    historyEmpty: 'No activations yet',
    activeDiscount: {
      personal: 'Personal discount {{percent}}% is active',
      purchase: 'Next-purchase discount {{percent}}% is active',
      hint: 'The discount applies automatically at checkout.',
    },
    historyMeta: {
      status: {
        active: 'Active',
        expired: 'Expired',
        applied: 'Applied',
      },
      units: {
        days: 'd',
        gb: 'GB',
        devices: 'dev.',
      },
      rewardTypes: {
        DURATION: 'Subscription days',
        TRAFFIC: 'Traffic',
        DEVICES: 'Devices',
        SUBSCRIPTION: 'Subscription',
        PERSONAL_DISCOUNT: 'Personal discount',
        PURCHASE_DISCOUNT: 'Purchase discount',
      },
    },
    successDefault: 'Promo code activated successfully!',
    invalid: 'Promo code is invalid',
    activationError: 'Activation error',
    done: 'Done!',
    toHome: 'Back to home',
    tip: 'Enter a promo code to get a discount or extra subscription time.',
    inputPlaceholder: 'ENTER CODE',
    successByType: {
      DURATION: 'Extra days added to your subscription.',
      TRAFFIC: 'Extra traffic added to your subscription.',
      DEVICES: 'Extra devices added to your subscription.',
      SUBSCRIPTION: 'Subscription activated.',
      PERSONAL_DISCOUNT: 'Personal discount activated.',
      PURCHASE_DISCOUNT: 'Next-purchase discount activated.',
      default: 'Promo code activated successfully!',
    },
    errors: {
      NOT_FOUND: 'Promo code not found.',
      INACTIVE: 'This promo code is disabled.',
      EXPIRED: 'This promo code has expired.',
      DEPLETED: 'This promo code has reached its activation limit.',
      ALREADY_ACTIVATED: 'You have already activated this promo code.',
      NOT_AVAILABLE_FOR_USER: 'This promo code is not available for your account.',
      PLAN_NOT_ELIGIBLE: 'This promo code does not apply to the selected subscription.',
      SUBSCRIPTION_NOT_FOUND: 'Subscription not found.',
      SUBSCRIPTION_NOT_ACTIVE: 'The subscription is not active.',
      SUBSCRIPTION_FOREIGN: 'The subscription belongs to another user.',
      NO_ELIGIBLE_SUBSCRIPTION: 'No active subscription to apply this to. Get a subscription first.',
      REWARD_NOT_APPLICABLE: 'This promo code reward cannot be applied.',
      INTERNAL_ERROR: 'Could not activate the promo code. Please try again later.',
      default: 'Promo code is invalid',
    },
    selectSubscription: {
      title: 'Choose a subscription',
      description: 'Which subscription should the promo code apply to?',
    },
    createNew: {
      title: 'Create a new subscription?',
      description: 'You have no active subscription. The promo code will create a new one.',
      confirm: 'Create',
      cancel: 'Cancel',
    },
    subscriptionLabel: 'Subscription',
  },
  referrals: {
    title: 'Referrals',
    subtitle: 'Invite friends and earn bonuses',
    invited: 'Invited',
    qualified: 'Qualified',
    getLink: 'Get referral link',
    copyLink: 'Copy link',
    createNew: 'Create new link',
    copied: 'Link copied!',
    copyError: 'Failed to create link',
    yourLink: 'Your referral link',
    qrTitle: 'QR code',
    howItWorks: 'How it works',
    howItWorksSub: 'Referral program terms',
    invitedList: 'Invited users',
    invitedEmpty: "You haven't invited anyone yet",
    pending: 'Pending',
    step1: 'Copy the referral link',
    step2: 'Share with a friend',
    step3: 'Friend registers and pays for subscription',
    step4: 'You get a bonus!',
    share: 'Share',
    shareText: 'Join {{brand}}!',
    points: 'Points',
    pointsBalance: 'Your points balance',
    pointsHint: 'Points are earned when your invited friend pays for a subscription. Exchange them for subscription days, discounts, and other bonuses.',
    pointsHintCashback: 'Points are earned on your own purchases and on the friends you invite. Exchange them for subscription days, discounts, and other bonuses.',
    qualifiedHint: 'Qualified friends are those who paid for a subscription after signing up through your link.',
    exchangePoints: 'Exchange points',
    inviteSlots: 'Invite slots',
    inviteSlotsRemaining: '{{count}} remaining',
    inviteLinkUnavailable: 'An invite link can’t be issued right now. Try again later, or check whether you have invites left.',
    inviteSlotsFull: 'All slots used',
    // Desktop sidebar card. The reward unit is the operator’s choice, so
    // both forms exist and neither is a default.
    cardTitle: 'Invite a friend',
    cardRewardPoints_one: '+{{count}} point per friend',
    // `_few` / `_many` exist here only because `RuDict` is the contract and
    // Russian needs them; English resolves `_other` for both.
    cardRewardPoints_few: '+{{count}} points per friend',
    cardRewardPoints_many: '+{{count}} points per friend',
    cardRewardPoints_other: '+{{count}} points per friend',
    cardRewardDays_one: '+{{count}} subscription day per friend',
    cardRewardDays_few: '+{{count}} subscription days per friend',
    cardRewardDays_many: '+{{count}} subscription days per friend',
    cardRewardDays_other: '+{{count}} subscription days per friend',
  },
  partner: {
    title: 'Partner',
    subtitle: 'Earn from bringing new customers',
    level: 'Level',
    levelHint: '% from payments',
    levelDescription: 'Your current partner level determines the percentage you earn from each payment by your referrals.',
    levelPercent: 'Level {{level}}',
    referrals: 'Referrals',
    balance: 'Balance',
    earned: 'Earned: {{amount}}',
    totalEarned: 'Total earned',
    info: 'Terms',
    infoDescription: 'The partner program lets you earn a percentage of every payment made by users you refer.',
    step1: 'Share your link — a friend signs up through it',
    step2: 'Friend pays for a subscription — you earn a percentage',
    step3: 'Withdraw your earnings via your preferred method',
    withdraw: 'Withdraw funds',
    recentEarnings: 'Recent earnings',
    referralsList: 'Referred users',
    referralsEmpty: 'No one referred yet',
    pointsHint: 'Points earned before you became a partner. No new ones accrue — exchange what is left.',
  },
  partnerBalanceHold: {
    notice:
      'Your partner balance is on hold until {{until}}. The account password was recently recovered with the subscription link. Someone else could have used that link, so after such a recovery the balance can’t be spent or withdrawn for a while. Once the hold ends, the balance is available again.',
    refused: 'Your partner balance is on hold after the password was recovered with the subscription link.',
    refusedUntil: 'Your partner balance is on hold until {{until}} after the password was recovered with the subscription link.',
  },
  partnerAds: {
    title: 'Advertising',
    reviewNote: 'Operator comment: {{note}}',
    trackingOff:
      'This placement is "{{status}}": visits through these links are not counted. Ask the operator to activate it again.',
    placementStatus: {
      DRAFT: 'draft',
      ACTIVE: 'active',
      PAUSED: 'paused',
      ARCHIVED: 'archived',
    },
    subtitle: 'Promote the service on your channels and earn from the customers you bring.',
    noPlacements: 'No active placements yet. Submit a request below.',
    opens: 'Opens',
    regs: 'Sign-ups',
    conv: 'Purchases',
    earned: 'Earned',
    newRequest: 'New advertising request',
    platformsHint: 'Pick the platforms where you will advertise. We issue a separate link for each.',
    channelPlaceholder: 'Channel / blog link (optional)',
    windowLabel: 'Attribution window (days)',
    windowHint: 'How many days after a click a purchase still counts for you. The operator confirms the final value.',
    submit: 'Submit request',
    submitted: 'Request submitted for review',
    submitError: 'Failed to submit the request',
    linksTitle: 'Your tracking links',
    payload: 'Code',
    linkBot: 'Bot',
    linkMiniApp: 'Mini App',
    linkWeb: 'Web',
    qrBot: 'QR → bot',
    qrWeb: 'QR → web',
    qrEnlargeBot: 'Open the bot QR code full size',
    qrEnlargeWeb: 'Open the web link QR code full size',
    qrDialogBot: 'Bot QR code',
    qrDialogWeb: 'Web link QR code',
    copy: 'Copy',
    copied: 'Copied',
    copyError: 'Could not copy',
    accept: 'Accept terms',
    accepted: 'Terms accepted — placements activated',
    acceptError: 'Failed to accept terms',
    counterHint: 'Operator proposes {{approved}} days (you asked for {{proposed}}).',
    platforms: {
      TELEGRAM: 'Telegram',
      TELEGRAM_ADS: 'Telegram Ads',
      YOUTUBE: 'YouTube',
      TIKTOK: 'TikTok',
      INSTAGRAM: 'Instagram',
      VK: 'VK',
      WEBSITE: 'Website',
      INFLUENCER: 'Influencer',
      OTHER: 'Other',
    },
    status: {
      PENDING: 'Under review',
      APPROVED: 'Approved',
      COUNTERED: 'Counter terms',
      ACCEPTED: 'Accepted',
      ACTIVE: 'Active',
      REJECTED: 'Rejected',
      EXPIRED: 'Expired',
    },
  },
  partnerWithdraw: {
    title: 'Withdraw funds',
    available: 'Balance: {{amount}}',
    amountLabel: 'Amount',
    amountLimits: 'You can withdraw from {{min}} to {{max}} — no more than your current balance.',
    amountAll: 'Whole balance',
    methodLabel: 'Payout method',
    methods: {
      card: 'Bank card',
      sbp: 'SBP (Faster Payments)',
      crypto: 'Cryptocurrency',
      other: 'Another method',
    },
    requisitesLabel: {
      card: 'Card number and bank',
      sbp: 'Phone number and recipient’s bank',
      crypto: 'Coin, network and wallet address',
      other: 'Where to send the money',
    },
    requisitesPlaceholder: {
      card: '0000 0000 0000 0000, bank',
      sbp: '+7 900 000-00-00, bank',
      crypto: 'USDT, TRC-20, address',
      other: 'Describe where and how to send it',
    },
    requisitesHint: 'The administrator sends the money using these details, so check them carefully.',
    howItWorks:
      'An administrator reviews the request and sends the money manually. The amount leaves your balance right away and comes back if the request is rejected.',
    next: 'Next',
    confirmTitle: 'Check your request',
    rowAmount: 'Amount',
    rowMethod: 'Method',
    rowRequisites: 'Details',
    rowStatus: 'Status',
    rowCreated: 'Created',
    confirmNote:
      'Once you send it, {{amount}} leaves your balance and waits for the payout. If the administrator rejects the request, the money comes back to your balance.',
    submit: 'Send request',
    edit: 'Change',
    doneTitle: 'Request sent',
    doneBody:
      'An administrator will review it and send the money manually. You can follow its status under “Balance” on this page.',
    done: 'Done',
    errors: {
      amountRequired: 'Enter an amount.',
      amountInvalid: 'Enter the amount as a number, for example 1500 or 1500.50.',
      amountTooSmall: 'The minimum is {{min}}.',
      amountTooLarge: 'That is more than your balance. Available: {{max}}.',
      requisitesRequired: 'Say where to send the money.',
      requisitesTooLong: 'Too long — {{max}} characters at most.',
    },
    refused: {
      insufficient: 'Your balance is now {{balance}}, which is less than the request. Lower the amount.',
      belowMinimum: 'The minimum withdrawal is {{min}}. Raise the amount.',
      inactive: 'Your partner account is switched off, so you can’t withdraw right now. Please contact support.',
      invitedOnly:
        'Withdrawals are unavailable: the partner programme is currently open only to invited users. Please contact support.',
      notPartner: 'You are no longer in the partner programme, so withdrawals are unavailable.',
      failed:
        'The request could not be sent. Before trying again, check the list of requests under “Balance”: if the new request is there, don’t send it again.',
    },
    empty: 'There is nothing on your balance to withdraw yet.',
    belowMinimum: 'You can withdraw from {{min}}; your balance is {{balance}} so far.',
    history: {
      title: 'Withdrawal requests',
      statuses: {
        PENDING: 'Under review',
        COMPLETED: 'Paid',
        REJECTED: 'Rejected',
        CANCELED: 'Cancelled',
      },
      unknownStatus: 'Status: {{status}}',
      requested: 'Requested {{date}}',
      processed: 'Processed {{date}}',
      comment: 'Administrator’s comment: {{comment}}',
      refunded: 'The amount is back on your balance.',
    },
  },
  settings: {
    title: 'Settings',
    profile: 'Profile',
    language: 'Language',
    themeMode: 'Appearance',
    themeModeDescription: 'Choose the light or dark representation of the concept selected by the operator.',
    themeModeLight: 'Light',
    themeModeDark: 'Dark',
    notifications: 'Notifications',
    notificationsSub: 'Feed and notification settings',
    notificationsSettings: 'Notification settings',
    notificationsSettingsSub: 'Browser notifications and subscription alerts',
    emailLink: 'Email',
    password: 'Password',
    changeLanguage: 'Change language',
    changePassword: 'Change password',
    signOut: 'Sign out',
    signOutConfirm: 'Are you sure you want to sign out?',
    languageUpdated: 'Language updated',
    languageUpdateError: 'Failed to update language',
    statusActive: 'Active',
    privacy: 'Privacy',
    privacySub: 'Password, Telegram & Email linking',
    transactions: 'Transactions',
    transactionsSub: 'Payment history',
    paymentMethods: 'Payment methods',
    paymentMethodsSub: 'Saved cards and unbind',
    addons: 'My add-ons',
    addonsSub: 'Purchased add-ons and their status',
    promocodes: 'Promo codes',
    promocodesSub: 'Activate and view promo history',
    support: 'Support',
    supportSub: 'Create a ticket, chat with support',
    faq: 'Help',
    faqSub: 'Frequently asked questions',
    installApp: 'Install app',
    installAppSub: 'Add the cabinet to your home screen',
    installPromptTitle: 'Install {{brand}}',
    installPromptIntro:
      'The app lands on your home screen and opens without a browser address bar.',
    installPromptAction: 'Install',
    installUnavailableTitle: 'Installing is not available here',
    installUnavailableIntro:
      'This browser does not offer installation. Open the cabinet in Chrome or Safari and the option appears.',
    installAlreadyTitle: 'The app is already installed',
    installAlreadyIntro:
      'You opened the cabinet from the installed app. If this was a quest, the reward becomes available within a few minutes.',
    installIosTitle: 'Install {{brand}}',
    installIosIntro: 'In Safari, add the cabinet to your home screen:',
    installIosStep1: 'Tap the Share button',
    installIosStep2: 'Choose "Add to Home Screen"',
    installIosStep3: 'Confirm — the icon appears on your screen',
    installAppSubTelegram: 'Open it in a browser to install',
    installTelegramTitle: 'Install {{brand}}',
    installTelegramIntro: 'Installing from inside Telegram is not possible: the Mini App runs in a browser built into Telegram, and that browser has no way to add an app to your device.',
    installTelegramStep1: 'Open this address in a normal browser — Safari on iPhone, Chrome on Android:',
    installTelegramStep2: 'Sign in there and come back to this settings section — installing will be available.',
    replayTutorial: 'Replay tutorial',
    replayTutorialSub: 'Take the cabinet intro tour again',
  },
  paymentMethods: {
    setupTitle: 'Add a card for autopay',
    setupHint: 'YooKassa verifies and saves the card without charging it. You can disable autopay or unbind the card at any time.',
    addCard: 'Add card',
    setupPending: 'Checking the card binding…',
    setupConfirmTitle: 'Save this card?',
    setupConfirmBody: 'Card details are entered on YooKassa’s secure page and never reach us.',
    setupConsent: 'I agree to save this payment method for future autopayments and understand that I can disable them in settings.',
    continueToYookassa: 'Continue to YooKassa',
    setupSuccess: 'Card added for autopay',
    setupFailed: 'The card could not be linked. Please try again.',
    setupError: 'Could not start card linking. Try again later.',
    setupStatusUnavailable: 'Could not verify the binding status. If you finished linking, the card will appear in the list shortly.',
    hint: 'Saved payment methods and automatic charging. You can disable autopay without unbinding a card, or unbind it completely — no support ticket needed.',
    empty: 'No saved payment methods',
    unbind: 'Unbind',
    unbinding: 'Unbinding...',
    unbindTitle: 'Unbind payment method?',
    unbindConfirm: 'After unbinding, this method will no longer be used for autopayments. You can save a card again on the next checkout.',
    unbound: 'Payment method unbound',
    error: 'Failed to unbind payment method',
    unbindBusy: 'A payment with this method is in progress — you can unbind it in a minute',
    autopay: 'Autopay',
    autopayOnHint: 'This card can be charged for autopayments without re-entering details',
    autopayOffHint: 'Autopay is off for this card — it stays linked',
    autopayEnabled: 'Autopay enabled',
    autopayDisabled: 'Autopay disabled',
    autopayError: 'Failed to update autopay',
    autopayBusy: 'A payment with this method is in progress — you can change autopay in a minute',
    fallbackTitle: 'Payment method',
    providerSubscriptions: {
      fallbackPlan: 'Subscription',
      everyUnit: {
        day: 'every day',
        week: 'every week',
        month: 'every month',
        year: 'every year',
      },
      everyDay_one: 'every {{count}} day',
      everyDay_few: 'every {{count}} days',
      everyDay_many: 'every {{count}} days',
      everyDay_other: 'every {{count}} days',
      everyWeek_one: 'every {{count}} week',
      everyWeek_few: 'every {{count}} weeks',
      everyWeek_many: 'every {{count}} weeks',
      everyWeek_other: 'every {{count}} weeks',
      everyMonth_one: 'every {{count}} month',
      everyMonth_few: 'every {{count}} months',
      everyMonth_many: 'every {{count}} months',
      everyMonth_other: 'every {{count}} months',
      everyYear_one: 'every {{count}} year',
      everyYear_few: 'every {{count}} years',
      everyYear_many: 'every {{count}} years',
      everyYear_other: 'every {{count}} years',
      via: 'Automatic charging over SBP',
      nextCharge: 'Next charge {{date}}',
      pastDue: 'The last charge failed. To keep your access, renew manually or turn automatic charging off.',
      cancel: 'Turn off automatic charging',
      cancelling: 'Turning off…',
      cancelTitle: 'Turn off automatic charging?',
      cancelConfirm: 'Your bank will stop charging you. The paid term stays, and you can renew manually.',
      cancelled: 'Automatic charging turned off',
      cancelError: 'Could not turn off automatic charging. Please try again.',
    },
    expires: 'exp {{month}}/{{year}}',
    types: {
      bank_card: 'Bank card',
      yoo_money: 'YooMoney',
      sberbank: 'SberPay',
      tinkoff_bank: 'T-Pay',
      sbp: 'SBP',
      unknown: 'Other',
    },
  },

  register: {
    title: 'Register',
    subtitle: 'Create an account to access VPN',
    usernameLabel: 'Username',
    usernamePlaceholder: 'username',
    passwordLabel: 'Password',
    passwordPlaceholder: 'At least 8 characters',
    submit: 'Register',
    submitting: 'Registering...',
    legal: {
      accept: 'I accept the',
      retry: 'Try again',
      required: 'The service terms have changed. Tick the boxes and try again.',
      unavailable:
        'Cannot load the terms right now. Registration requires accepting them — please try again in a moment.',
    },
    hasAccount: 'Already have an account?',
    signIn: 'Sign in',
    disabled: 'Registration is temporarily unavailable. Please sign in to an existing account.',
    checkingUsername: 'Checking availability...',
    showPassword: 'Show password',
    hidePassword: 'Hide password',
    errorUsernameTaken: 'This username is already taken',
    errorDisabled: 'Registration is disabled',
    errorRateLimit: 'You have exceeded the registration limit. Please try again in {{seconds}} sec.',
    errorServiceUnavailable: 'Service temporarily unavailable. Please retry in 30 seconds.',
    usernameError: {
      required: 'Enter a username',
      tooShort: 'At least 3 characters',
      tooLong: 'Maximum 32 characters',
      invalidChars: 'Only letters, numbers, hyphens, and underscores',
    },
    passwordError: {
      required: 'Enter a password',
      tooShort: 'At least 8 characters',
      tooLong: 'Maximum 128 characters',
    },
  },
  finishSetup: {
    title: 'Finish registration',
    subtitle: 'Create a login and password — you will need them to sign in, including in the Telegram app.',
    usernameLabel: 'Login',
    usernamePlaceholder: 'login',
    passwordLabel: 'Password',
    passwordPlaceholder: 'At least 8 characters',
    submit: 'Finish',
    submitting: 'Saving...',
    cancel: 'Cancel',
    haveAccount: 'Already have an account? Sign in',
    showPassword: 'Show password',
    hidePassword: 'Hide password',
    errorUsernameTaken: 'Looks like you already have an account with this login. Sign in to it — you can link Telegram later in settings.',
    errorRateLimit: 'You have exceeded the limit. Please try again in {{seconds}} sec.',
    errorGeneric: 'Could not save. Please try again.',
    usernameError: {
      required: 'Enter a login',
      tooShort: 'At least 3 characters',
      tooLong: 'At most 32 characters',
      invalidChars: 'Only letters, digits, hyphen and underscore',
    },
    passwordError: {
      required: 'Enter a password',
      tooShort: 'At least 8 characters',
      tooLong: 'At most 128 characters',
    },
  },
  claim: {
    title: 'Set up your account',
    subtitle: 'Create a login and password so you can always sign in from a browser — even without Telegram.',
    usernameLabel: 'Login',
    usernamePlaceholder: 'login',
    passwordLabel: 'Password',
    passwordPlaceholder: 'At least 8 characters',
    submit: 'Continue',
    submitting: 'Saving...',
    showPassword: 'Show password',
    hidePassword: 'Hide password',
    errorUsernameTaken: 'This login is already taken',
    errorRateLimit: 'You have exceeded the limit. Please try again in {{seconds}} sec.',
    errorServiceUnavailable: 'Service temporarily unavailable. Please retry in 30 seconds.',
    errorGeneric: 'Could not save. Please try again.',
    usernameError: {
      required: 'Enter a login',
      tooShort: 'At least 3 characters',
      tooLong: 'Maximum 32 characters',
      invalidChars: 'Only letters, numbers, hyphens, and underscores',
    },
    passwordError: {
      required: 'Enter a password',
      tooShort: 'At least 8 characters',
      tooLong: 'Maximum 128 characters',
    },
    linkExisting: {
      prompt: 'Already have an account and used the service?',
      action: 'Log in',
      back: 'Create a new account instead',
      title: 'Log in to your account',
      subtitle: 'Enter your existing login and password — we’ll link this Telegram to that account and bring over your subscription.',
      submit: 'Log in and link',
      errorInvalid: 'Invalid login or password.',
      errorNeedsMerge: 'This Telegram already has its own history. Please contact support to merge the accounts.',
      errorOtherTelegram: 'This account is already linked to a different Telegram account.',
    },
  },
  auth: {
    signIn: 'Sign In',
    signInTitle: 'Sign in to your account',
    signInDescription: 'Enter your username and password',
    username: 'Username',
    usernamePlaceholder: 'Enter username',
    password: 'Password',
    passwordPlaceholder: 'Enter password',
    signInButton: 'Sign In',
    signingIn: 'Signing in...',
    invalidCredentials: 'Invalid username or password',
    rateLimited: 'You have exceeded the login limit. Please try again in {{seconds}} sec.',
    noAccount: "Don't have an account?",
    register: 'Register',
    orContinueWith: 'or continue with',
    continueWith: 'Continue with {{provider}}',
    errorExternal: 'External sign-in failed. Please try again.',
    errorExternalDenied: 'Access denied.',
    errorExternalEmailUnverified:
      'An account with this email already exists, but the email was never confirmed on it, so signing in to it with Google, Yandex or Mail.ru is not possible. Sign in with your login or through Telegram, or contact support.',
    sessionCheckUnavailable:
      'Could not confirm that you are still signed in. Nothing was changed — try again in a minute.',
    forgotPassword: 'Forgot password?',
    passwordNotSet: {
      sentTelegram: 'This account has no password yet. We have sent a link to your Telegram — open it and set a password.',
      sentEmail: 'This account has no password yet. We have sent a link to your email — open it and set a password.',
      hourlyLimit: 'This account has no password yet. Links for it have already been sent — open the latest one and set a password. You can request a new one later.',
      useBot: 'This account has no password yet. Open the bot — it will send you a link to set one.',
      unavailable: 'This account has no password yet. We could not send a link right now — please try signing in again in a minute.',
      openBot: 'Get a link in Telegram',
    },
    recover: {
      title: 'Password recovery',
      description: 'Enter your login or the email linked to your account, and we will send you a password reset link.',
      identifierLabel: 'Login or email',
      identifierPlaceholder: 'Login or email',
      identifierRequired: 'Enter your login or email',
      submit: 'Get a link',
      submitting: 'Sending…',
      backToSignIn: 'Back to sign in',
      sent: 'If this login exists and has Telegram or an email linked, we have sent a reset link. It is valid for 15 minutes.',
      sentHint: 'Check the bot’s messages in Telegram and your inbox, including the spam folder.',
      unavailable: 'Recovery by link is not available on this server yet. Contact support and they will help you sign in.',
      rateLimited: 'Too many requests. Please try again in {{seconds}} sec.',
      error: 'Could not send the request. Please try again.',
      telegramShortcut: 'Forgot your login? If Telegram is linked to your account, the bot will send you a link and remind you of your login.',
      telegramShortcutAction: 'Get a link in Telegram',
      noAccessTitle: 'No access to Telegram or email?',
      noAccessBody: 'If neither Telegram nor an email is linked to your account, you can set a new password with your VPN subscription link. If one is linked, the reset link will be sent there.',
      noAccessAction: 'Recover with the subscription link',
      noAccessSupport: 'Lost access to the linked Telegram or email, or forgot your login too? Contact support.',
      noAccessSupportOnly: 'Contact support — they will help you get back into your account.',
    },
    reset: {
      title: 'New password',
      subtitle: 'For the login {{login}}',
      checking: 'Checking the link…',
      newPassword: 'New password',
      newPasswordPlaceholder: 'At least 8 characters',
      repeatPassword: 'Repeat the password',
      repeatPasswordPlaceholder: 'Once more',
      showPassword: 'Show password',
      hidePassword: 'Hide password',
      lengthError: 'The password must be 8 to 128 characters long',
      mismatch: 'The passwords do not match',
      submit: 'Save password',
      submitting: 'Saving…',
      expiredTitle: 'The link has expired',
      expiredBody: 'A reset link is valid for 15 minutes. Request a new one — it only takes a minute.',
      usedTitle: 'This link no longer works',
      usedBody: 'The password has already been changed with this link or in another way. If it wasn’t you, request a new link and contact support.',
      missingTitle: 'The link is incomplete',
      missingBody: 'Open the full link from the message, or request a new one.',
      requestNew: 'Request a new link',
      rateLimited: 'Too many attempts. Please try again in {{seconds}} sec.',
      error: 'Could not save the password. Please try again.',
      unavailable: 'The service is temporarily unavailable. Please try again in a minute.',
      changedTitle: 'Password changed — sign in with your new password',
      changedLogin: 'Your login: {{login}}',
      signIn: 'Sign in',
    },
    recoverSubscription: {
      title: 'Recovery with the subscription link',
      description: 'For an account without Telegram or email: paste the subscription link from your VPN app and enter your cabinet login. If Telegram or an email is linked to the account, the reset link will be sent there.',
      linkLabel: 'Subscription link',
      linkPlaceholder: 'https://… or a link from the app',
      linkHint: 'You can copy it in the app where your VPN is set up, usually in the subscription settings. An encrypted Happ link (happ://crypt…) will not work — use the plain one.',
      loginLabel: 'Login',
      loginPlaceholder: 'Your cabinet login',
      required: 'Fill in both fields',
      submit: 'Confirm',
      submitting: 'Checking…',
      notVerified: 'Could not confirm — check the link and the login.',
      lockedHint: 'After several failed attempts this method is blocked for a while — then contact support.',
      rateLimited_one: 'Too many attempts. Please try again in {{count}} minute.',
      rateLimited_few: 'Too many attempts. Please try again in {{count}} minutes.',
      rateLimited_many: 'Too many attempts. Please try again in {{count}} minutes.',
      rateLimited_other: 'Too many attempts. Please try again in {{count}} minutes.',
      unavailable: 'This method is not available right now. Please try later or contact support.',
      error: 'Could not check. Please try again.',
      forgotLogin: 'Forgot your login too? This method will not work — contact support.',
      back: 'Back',
      loading: 'Checking whether this option is available…',
      disabledTitle: 'Recovery by subscription link is turned off',
      disabledBody: 'This option is not available right now. Contact support — they will help you get back into your account.',
    },
    saveCredentials: {
      title: 'Save your sign-in details',
      subtitle: 'You will need them to sign in from another device or a browser.',
      loginLabel: 'Login',
      passwordLabel: 'Password',
      addressLabel: 'Cabinet address',
      show: 'Show',
      hide: 'Hide',
      copy: 'Copy',
      copied: 'Copied: login, password and cabinet address',
      copyFailed: 'Could not copy. Select the details and copy them by hand.',
      share: 'Share',
      shareFailed: 'Could not open the share menu.',
      shareTitle: 'Sign-in details',
      shareText: 'Login: {{login}}\nPassword: {{password}}\nCabinet: {{address}}',
      saveInBrowser: 'Save in browser',
      savedInBrowser: 'The password was handed to your browser. If it asks, confirm saving it.',
      saveInBrowserFailed: 'The browser did not save the password. Save it another way.',
      hintIos: 'On iPhone, keep the password in the Passwords app (iCloud Keychain) — Safari offers to save it for you.',
      hintAndroid: 'On Android, you can keep the password in Google Password Manager — Chrome offers to save it for you.',
      hintDesktop: 'Save the password in your browser’s password manager.',
      recoveryTitle: 'To recover your password without support',
      recoveryBody: 'Link Telegram or an email — then, if you lose your password, we will send you a reset link.',
      recoveryBodyTelegram: 'Link Telegram — then, if you lose your password, the bot will send you a reset link.',
      recoveryBodyEmail: 'Link an email — then, if you lose your password, we will send a reset link to it.',
      linkTelegram: 'Link Telegram',
      linkEmail: 'Link email',
      continue: 'Continue',
    },
  },
  changePassword: {
    errorCurrentWrong: 'Current password is incorrect.',
    errorPolicy: 'The new password does not meet the requirements. Check its length and contents.',
    errorRetryLater: 'The service is temporarily unavailable. Try again in 30 seconds.',
    title: 'Change password',
    description: 'Enter the temporary password your administrator issued, then set a new one.',
    currentPassword: 'Current password',
    currentPasswordPlaceholder: 'Temporary password',
    newPassword: 'New password',
    newPasswordPlaceholder: 'At least 8 characters',
    passwordLengthError: 'Password must be 8 to 128 characters',
    submit: 'Change password',
    success: 'Password changed. Welcome!',
    errorGeneric: 'Could not change the password. Check your current password and try again.',
  },
  firstPassword: {
    title: 'Set a password',
    description:
      'This account has no password yet. Choose one — you will use it to sign in to the cabinet in a browser too.',
    submit: 'Save password',
    success: 'Password set.',
    checking: 'Checking your account…',
    errorAlreadySet: 'This account already has a password. Enter it as the current one, or recover it.',
    errorNoAccount: 'A password cannot be set for this account here. Please contact support.',
  },
  signOutEverywhere: {
    label: 'Sign out on all devices',
    sub: 'Except this one — you stay signed in here',
    title: 'Sign out on all devices?',
    body:
      'Every other device and browser signed in to your account will be signed out within a minute. You stay signed in on this device. If someone may know your password, change it — that signs the other sessions out too.',
    confirm: 'Sign out on other devices',
    cancel: 'Cancel',
    done: 'Done: your other devices will be signed out within a minute.',
    unavailable: 'This is not available for your account right now.',
    failed: 'That did not work. Please try again a little later.',
  },
  nav: {
    home: 'Home',
    subscription: 'Subscription',
    plans: 'Plans',
    activity: 'Activity',
    settings: 'Settings',
  },
  bottomNav: {
    subscriptions: 'Subscriptions',
    referral: 'Referral',
    partner: 'Partner',
    settings: 'Settings',
    support: 'Support',
    plans: 'Plans',
    devices: 'Devices',
    activity: 'Activity',
    promo: 'Promo codes',
    faq: 'Help',
  },
  card: {
    activeStatus: 'Active',
    expiredStatus: 'Expired',
    pendingStatus: 'Pending',
    expiresOn: 'Expires',
    remaining: 'Remaining',
    daysLeft_one: '{{count}} day',
    daysLeft_few: '{{count}} days',
    daysLeft_many: '{{count}} days',
    daysLeft_other: '{{count}} days',
    /** Under a day to the end: whole hours, then minutes — as the add-on lines count. */
    hoursLeft_one: '{{count}} hour',
    hoursLeft_few: '{{count}} hours',
    hoursLeft_many: '{{count}} hours',
    hoursLeft_other: '{{count}} hours',
    minutesLeft_one: '{{count}} minute',
    minutesLeft_few: '{{count}} minutes',
    minutesLeft_many: '{{count}} minutes',
    minutesLeft_other: '{{count}} minutes',
    untilDate: 'until {{date}}',
    firstDevice: 'Device',
    noDevicesYet: 'No devices yet',
    /** Device read failed — the card slot must not claim "No devices yet". */
    devicesUnavailable: 'Unavailable',
    trafficUsed: 'Used',
    trafficUnlimited: 'Unlimited traffic',
    actions: {
      buy: 'Buy',
      connect: 'Connect',
      upgrade: 'Upgrade',
      renew: 'Renew',
      promo: 'Promo',
      topUp: 'Options',
    },
    pageOf: '{{current}} of {{total}}',
  },
  connect: {
    pageTitle: 'Connect',
    subtitle: 'Choose an app',
    platform: 'Platform',
    copyLink: 'Copy the link',
    copyHint: 'The link appears once the subscription is ready',
    copied: 'Link copied',
    copyFailed:
      'Could not copy the link. Scan the QR code in the app instead — the link icon at the top of the screen opens it.',
    noLink: 'This subscription has no link yet',
    catalogUnavailable:
      'The app list is unavailable right now. Copy the link above and paste it into your VPN client.',
    linkFailedTitle: 'Could not load the link',
    linkFailedBody: 'A temporary problem on our side. Try again in a minute.',
    linkUnknown: 'Could not load the link — try again',
    noSectionForDevice: 'There are no instructions for your device yet. Pick a platform by hand.',
    appsForPlatform: 'Apps for this platform',
    install: 'Installation',
    installHint: 'Pick an app and follow the steps',
    subscriptionMissing:
      'This subscription is unavailable — it may have been cancelled, or the link is out of date. Go back and pick a subscription again.',
    support: 'Support',
    recommended: 'Recommended',
    linkSheetOpen: 'Link and QR code',
    linkSheetHeading: 'Get the link',
    linkSheetTitle: 'Scan the QR code in the app',
    linkSheetBody:
      'The simple way to add the subscription to any client. Or copy the link below and paste it into the client.',
    factName: 'Name',
    factStatus: 'Status',
    factExpires: 'Expires',
    factTraffic: 'Traffic',
    openHint: 'A page will open — tap "Open the app" on it.',
    openTitle: 'Add the subscription',
    openBody: 'Tap the button — the app opens and the subscription is added to it.',
    openButton: 'Open the app',
    openSource: 'Subscription address: {{host}}',
    openNotOpened:
      'The app did not open? Make sure it is installed, or copy the link and add the subscription in the app by hand.',
    openCopyFailed: 'Could not copy. Go back to Telegram and copy the link on the connect screen.',
    openChecking: 'Checking the link…',
    openFailed: 'Could not check the link. Check your internet connection and try again.',
    openInvalid:
      'The link is incomplete or damaged. Go back to Telegram and tap the add-subscription button again.',
    openUnverified:
      'This page could not confirm that the link was made on the connect screen of this service, so it will not open it. Go back to Telegram and copy the link on the connect screen.',
  },
  devices: {
    title: 'Connected devices',
    empty: 'No devices connected yet',
    lastSeen: 'Last seen {{when}}',
    revoke: 'Revoke',
    revokeConfirm: 'Revoke this device? Access will be blocked immediately.',
    revokeAll: 'Disconnect all',
    revokeAllConfirm:
      'All devices will be disconnected. Every device will need to be set up again. Continue?',
    copyLink: 'Copy',
    copied: 'Link copied',
    regenerate: 'Regenerate',
    regenerateConfirm:
      'The link will be regenerated and all devices on this subscription disconnected. Reconnect them with the new link. Continue?',
    regenerated: 'Link regenerated. Reconnect your devices.',
    regeneratedNotCleared:
      'New link created — the old one no longer works. Your devices could NOT be disconnected, so they still hold their slots: remove the ones you do not use, then connect with the new link.',
    allRevoked: 'All devices disconnected. Reconnect them.',
    revoked: 'Device revoked',
    error: 'Operation failed',
    pageTitle: 'Devices',
    connectedCount: '{{count}} connected',
    tip: 'Remove an old device to free up a slot for a new connection.',
    emptyTitle: 'No connected devices',
    emptyHint: 'Connect to VPN — the device will appear here',
    deviceFallback: 'Device',
    lastSeenLabel: 'Last seen: {{date}}',
    unlimited: 'Unlimited',
    multiInfoAria: 'How subscriptions and limits work',
    multiInfoTitle: 'Multi-subscription & limits',
    multiInfoBody: 'You can hold several subscriptions at once — swipe between the cards. Each subscription has its own profile, connect link, and limits.',
    multiInfoDeviceLimit: 'Device limit',
    multiInfoTrafficLimit: 'Traffic limit',
    multiInfoTrafficValue: '{{value}} GB',
    multiInfoHint: 'Limits are shown for the currently selected subscription. Devices and traffic are counted per subscription.',
    deleteConfirm: 'Remove this device?',
    deleted: 'Device removed',
    deleteError: 'Failed to remove device',
    // Shown INSTEAD of the empty state when the device read failed. Must not
    // read like "you have no devices" — it is our outage, not the customer's.
    loadFailedTitle: 'Could not load your devices',
    loadFailedBody:
      'This is a temporary problem on our side, not a change to your devices. Your connected devices are unchanged — try again in a moment.',
    countUnavailable: 'Device count unavailable',
  },
  subscriptionPicker: {
    traffic: 'Traffic',
    gb: 'GB',
    unlimited: 'Unlimited',
    expires: 'Expires',
    devices: 'up to {{count}} devices',
  },
  mediaViewer: {
    counter: '{{current}} of {{total}}',
    close: 'Close',
    previous: 'Previous',
    next: 'Next',
    expand: 'Expand',
    openImage: 'Open full screen: {{label}}',
  },
  addons: {
    title: 'Add-ons',
    description: 'Top up traffic or devices for your current subscription.',
    selectSubtitle: 'Choose a subscription to top up',
    noneActive: 'No active subscriptions available for add-ons.',
    back: 'Back',
    creating: 'Creating payment…',
    selectGateway: 'Payment method',
    selectSubscription: 'Subscription',
    noCompatibleGateway: 'No compatible payment method for this add-on.',
    reviewTitle: 'Confirm purchase',
    total: 'Total',
    confirm: 'Proceed to payment',
    confirmFree: 'Activate',
    empty: 'No add-ons available for this plan.',
    loadError: 'Could not load add-ons. Please try again.',
    retry: 'Retry',
    extraTraffic: '+{{value}} GB traffic',
    extraDevices_one: '+{{count}} device',
    extraDevices_few: '+{{count}} devices',
    extraDevices_many: '+{{count}} devices',
    extraDevices_other: '+{{count}} devices',
    resetTraffic: 'Reset used traffic to zero',
    resetTrafficFree_one: 'Reset traffic · {{count}} free left',
    resetTrafficFree_few: 'Reset traffic · {{count}} free left',
    resetTrafficFree_many: 'Reset traffic · {{count}} free left',
    resetTrafficFree_other: 'Reset traffic · {{count}} free left',
    resetTrafficNote: 'Your plan limit and any bought gigabytes stay',
    resetTrafficTitle: 'Traffic reset',
    resetTrafficConfirm: 'Used traffic drops to zero and counting starts again. This cannot be undone or given back.',
    resetTrafficKeeps: 'Your plan limit does not change. Gigabytes you bought stay and run to their own expiry.',
    resetTrafficRemaining_one: 'After this you will have {{count}} free reset left for the current subscription term.',
    resetTrafficRemaining_few: 'After this you will have {{count}} free resets left for the current subscription term.',
    resetTrafficRemaining_many: 'After this you will have {{count}} free resets left for the current subscription term.',
    resetTrafficRemaining_other: 'After this you will have {{count}} free resets left for the current subscription term.',
    resetTrafficAction: 'Reset traffic',
    resetTrafficDone: 'Traffic reset',
    resetTrafficFailed: 'Could not reset traffic. Please try again.',
    purchaseError: 'Failed to create payment. Please try again later.',
    free: 'Free',
    freeApplied: 'Add-on activated',
    validUntil: 'Valid until {{date}}',
    untilSubscriptionEnd: 'Until the subscription ends',
    // Which bound ends the add-on, as the panel says (`eligibility.endsBound`;
    // `endsBound` in «My add-ons»): the traffic reset, shown at the reset's own
    // moment on the operator's clock with the zone named, or the subscription's
    // end. An older panel sends no bound and keeps the two lines above.
    untilReset: 'Valid until the traffic reset on {{date}} at {{time}} ({{zone}})',
    untilSubscriptionEndOn: 'Valid until the subscription ends on {{date}}',
    endCountdown: '{{end}} — {{countdown}}',
    inDays_one: 'in {{count}} day',
    // `_few` / `_many` exist only because `RuDict` is the contract and Russian
    // needs them; English resolves `_other` for both.
    inDays_few: 'in {{count}} days',
    inDays_many: 'in {{count}} days',
    inDays_other: 'in {{count}} days',
    inHours_one: 'in {{count}} hour',
    inHours_few: 'in {{count}} hours',
    inHours_many: 'in {{count}} hours',
    inHours_other: 'in {{count}} hours',
    inMinutes_one: 'in {{count}} minute',
    inMinutes_few: 'in {{count}} minutes',
    inMinutes_many: 'in {{count}} minutes',
    inMinutes_other: 'in {{count}} minutes',
    // Before the payment, when the reset that ends the add-on is less than a
    // day away (`eligibility.resetSoon`). The price is the full price, so it is
    // said plainly.
    resetSoonWarning: 'The traffic resets {{countdown}} — the add-on ends with it',
  },
  addonsHistory: {
    title: 'My add-ons',
    empty: "You haven't purchased any add-ons yet.",
    expires: 'Valid until {{date}}',
    state: {
      ACTIVE: 'Active',
      PENDING_ACTIVATION: 'Pending activation',
      EXPIRING: 'Expiring',
      EXPIRED: 'Expired',
      REVERSED: 'Reversed',
      REMEDIATION_REQUIRED: 'Needs attention',
    },
  },
  renewal: {
    title: 'Renewal',
    back: '← Back',
    selectSubtitle: 'Choose subscriptions to renew',
    continue: 'Continue',
    durationLabel: 'Renewal term',
    choosePlanTitle: 'Choose a plan to renew with',
    choosePlanHint: 'Pick a plan',
    choosePlanCta: 'Choose plan',
    noneRenewable: 'No subscriptions are available to renew.',
    loadError: 'Could not load the subscriptions to renew.',
    reason: {
      trial: "A trial can't be renewed — upgrade to a regular plan instead.",
      noPlan: 'This subscription has no plan — pick a plan when renewing.',
      noGateway: 'No payment method is available right now. Contact support.',
      archived: 'The plan is archived — renewing moves it to an available plan.',
      lifetime: 'This subscription never expires — there is nothing to renew.',
    },
    unknownPlan: 'Plan',
    reviewTitle: 'Confirm renewal',
    total: 'Total',
    pay: 'Proceed to payment',
    change: 'Change',
    home: 'Back to home',
    payWithBalance: 'Pay with balance ({{amount}} {{currency}})',
    balancePaid: 'Paid from partner balance',
    balanceError: 'Failed to pay with balance',
    priceError: 'Could not calculate the price. Try a different payment method.',
    creating: 'Creating payment…',
    checkoutError: 'Failed to create payment. Please try again later.',
  },
  upgrade: {
    title: 'Upgrade',
    back: '← Back',
    selectSubtitle: 'Choose a subscription to upgrade',
    noneUpgradeable: 'No subscriptions available to upgrade.',
    noTargets: 'No upgrades are available for this subscription.',
    lifetime: 'This subscription never expires — only support can change its plan.',
    choosePlan: 'Choose a new plan',
    chooseDuration: 'Choose a term',
    devices: 'devices',
    gb: 'GB',
    unlimited: 'Unlimited',
    reviewTitle: 'Confirm upgrade',
    newPlan: 'New plan',
    total: 'Total',
    resetsExpiry: 'The upgrade starts immediately and resets the subscription expiry.',
    resetsExpiryWithRemainder:
      'The upgrade starts immediately: the term starts over, and the paid remainder of your current plan is added to it as days.',
    paidRemainder:
      'Paid remainder of your current plan: +{{days}} d on top of the new term — the exact number is counted at payment.',
    keepsAbovePlan: 'Above your plan: {{items}} — you keep this on the new plan too.',
    aboveDevices_one: '+{{count}} device',
    // `en` is typed as the Russian dictionary, so the Russian plural keys
    // exist here too; English never selects them.
    aboveDevices_few: '+{{count}} devices',
    aboveDevices_many: '+{{count}} devices',
    aboveDevices_other: '+{{count}} devices',
    aboveTraffic: '+{{value}} GB',
    aboveUnlimitedDevices: 'unlimited devices',
    aboveUnlimitedTraffic: 'unlimited traffic',
    keepsAddOns: 'Your add-ons stay: {{items}}.',
    addOnUntil: '{{item}} until {{date}}',
    addOnUntilEnd: '{{item}} until the subscription ends',
    pay: 'Proceed to payment',
    change: 'Change',
    priceError: 'Could not calculate the price. Try a different payment method.',
    creating: 'Creating payment…',
    checkoutError: 'Failed to create payment. Please try again later.',
  },
  subscriptionProvisioning: {
    creating: 'Creating subscription',
    calibrating: 'Configuring profile',
    waiting: 'Connecting profile',
    longWaiting: 'The profile is still being configured',
    ready: 'Subscription ready',
    failed: 'Could not configure the profile',
    failedHint: 'Your payment is saved. Refresh later or contact support.',
  },
  deleteSubscription: {
    open: 'Delete subscription',
    title: 'Delete subscription?',
    body: 'Do you really want to delete the subscription "{{name}}"?',
    warning: 'This is irreversible. Access stops immediately and there is no refund.',
    yes: 'Yes, delete',
    no: 'No',
    success: 'Subscription deleted',
    finishing: 'Finishing deletion…',
    error: 'Failed to delete the subscription. Please try again later.',
  },
  onboarding: {
    skip: 'Skip',
    next: 'Next',
    prev: 'Back',
    finish: 'Done',
    step1: {
      title: 'Your subscription',
      body: 'This is your active subscription. Profile name, expiry date and traffic — all on one card.',
    },
    stepDelete: {
      title: 'Deleting a subscription',
      body: 'To delete a subscription, press and hold the card, then confirm. This is irreversible: access is revoked and there is no refund.',
    },
    step2: {
      title: 'Subscription actions',
      body: 'Buy a new one, connect a device, or upgrade your plan — three buttons right under the card.',
    },
    step3: {
      title: 'Device management',
      body: 'See all your devices here. Revoke the ones you no longer use and regenerate the connection link.',
    },
    step4: {
      title: 'Referral & settings',
      body: 'Invite friends in the Referral tab. Language, security and notifications live in Settings.',
    },
    step5: {
      title: 'Ready to go?',
      body: "Let's start! You can replay this tour from Settings any time.",
    },
    demo: {
      badge: 'Example',
      title: 'How the cabinet works',
      intro: 'Here is a sample subscription so you can see how everything works before you start.',
      samplePlan: 'Demo plan',
      sampleStatus: 'Active',
      sampleExpiry: 'Expires in 30 days',
      sampleTraffic: '12 / 50 GB',
      sampleDevices: '2 of 5 devices',
      close: 'Got it',
    },
  },
  paymentAnim: {
    processing: 'Processing payment',
    waitingProvider: 'Waiting for the payment provider...',
    success: 'Payment successful',
    successHint: 'Your subscription is being activated. You can return to the app.',
    failed: 'Payment failed',
    failedHint: 'Try again or pick another payment method.',
    openPayment: 'Open payment page',
    openPaymentHint: "If the payment page didn't open automatically, tap the button below.",
    cashbackCredited_one: 'You earned +{{count}} point',
    cashbackCredited_few: 'You earned +{{count}} points',
    cashbackCredited_many: 'You earned +{{count}} points',
    cashbackCredited_other: 'You earned +{{count}} points',
    cashbackExchange: 'Exchange points',
  },
  paymentNumber: {
    label: 'Payment number',
    copy: 'Copy payment number',
    copied: 'Number copied',
    copyFailed: "Couldn't copy. Select the number and copy it by hand.",
  },
  privacy: {
    changePassword: 'Change password',
    changePasswordSub: 'Update your sign-in password',
    legalDocuments: 'Terms and conditions',
    legalDocumentsSub: 'The current wording — readable in full',
    legalDocumentsEmpty: 'The operator has not published any documents.',
    legalDocumentsNav: 'Document list',
    linkTelegram: 'Link Telegram',
    linkTelegramSub: 'For quick sign-in via bot',
    linkEmail: 'Link Email',
    linkEmailSub: 'For password recovery',
    linkTelegramHint: 'Tap the button below to get a linking code. Send it to the bot in Telegram.',
    linkEmailHint: 'Enter your email for password recovery. A verification code will be sent.',
    generateCode: 'Get linking code',
    sendVerification: 'Send code',
    currentPassword: 'Current password',
    newPassword: 'New password',
    passwordChanged: 'Password changed successfully',
    passwordError: 'Failed to change password. Check your current password.',
    linkedStatus: 'Linked',
    verifiedStatus: 'Verified',
    linkError: 'Operation failed. Please try again later.',
    yourCode: 'Your code',
    telegramCodeInstructions: 'Open the bot and send this code, or tap the button below — we will fill it in automatically. The code is valid for 10 minutes.',
    openBot: 'Open bot',
    regenerateCode: 'Get a new code',
    telegramAlreadyLinked: 'This Telegram is already linked to another account.',
    telegramLinkedHint: 'Your Telegram is already linked.',
    telegramRelinkHint: 'To link a different Telegram account, get a new code.',
    emailCodeSent: 'Code sent to your email',
    emailVerified: 'Email verified',
    emailVerifiedHint: 'Email {{email}} is verified.',
    emailRelinkHint: 'To change your email, enter a new address and verify it.',
    emailAlreadyLinked: 'This email is already linked to another account.',
    emailCodeHint: 'We sent a 6-digit code to {{email}}. Enter it below.',
    emailCodeInvalid: 'Invalid verification code.',
    emailCodeExpired: 'The code has expired. Request a new one.',
    emailTooManyAttempts: 'Too many attempts. The code was invalidated, request a new one.',
    verificationCode: 'Verification code',
  },
  notifications: {
    title: 'Notifications',
    feedTitle: 'Notifications feed',
    recentTitle: 'Latest news',
    seeAll: 'All notifications',
    loading: 'Loading notifications…',
    loadFailed: 'Could not load notifications. Tap to retry.',
    markAllReadFailed: 'Could not mark notifications as read. Please try again.',
    emptyBody: 'Notification text is unavailable.',
    feedSub: 'Received notifications',
    types: {
      broadcast: 'News',
      adminMessage: 'Message from the team',
      expiry: 'Subscription expiring soon',
      supportReply: '💬 Support replied',
      generic: 'Notification',
    },
    supportReplyBody: 'There is a new reply to your ticket "{{subject}}". Open the Support section.',
    expiryBody_one: 'Your {{plan}}subscription expires in {{count}} day. Renew to keep your access.',
    expiryBody_few: 'Your {{plan}}subscription expires in {{count}} days. Renew to keep your access.',
    expiryBody_many: 'Your {{plan}}subscription expires in {{count}} days. Renew to keep your access.',
    expiryBody_other: 'Your {{plan}}subscription expires in {{count}} days. Renew to keep your access.',
    expiryBodyGeneric: 'Your {{plan}}subscription is expiring soon. Renew to keep your access.',
    settingsTitle: 'Notification settings',
    settingsSub: 'Browser notifications and subscription alerts',
    beforeExpiry: 'Before subscription expires',
    afterExpiry: 'After subscription expires',
    prefsSaveFailed: 'Could not save the setting',
    days3: '3 days before',
    days2: '2 days before',
    days1: '1 day before',
    dayOf: 'On expiry day',
    after1: '1 day after',
    after2: '2 days after',
    after3: '3 days after',
    // A paid add-on — extra traffic or devices — before and at its end.
    addonsGroup: 'Add-ons',
    addonEndsSoon: '3 days before an add-on ends',
    addonEnded: 'When an add-on has ended',
    hint: 'Anything switched off here stops arriving in the bot, the browser and by email, but stays in Notifications — nothing is lost.',
    pushSection: 'Browser notifications',
    pushDescription: 'Get notifications in your browser or on your device, even when the site is closed.',
    pushBroadcastsHint: 'Important news and service announcements arrive here too — enable it so you never miss them.',
    pushToggleEnable: 'Enable browser notifications',
    pushToggleEnabled: 'Browser notifications are on',
    pushUnsupported: 'Your browser does not support push notifications.',
    pushIosInstall: 'To get notifications on iPhone or iPad, install the app to your Home Screen.',
    pushIosInstallHow: 'Open the Share menu and tap "Add to Home Screen".',
    pushIosInstallHowTelegram: 'This cannot be done from inside Telegram: open the cabinet in Safari and add it to the Home Screen from there.',
    pushPermissionDenied: 'Notification permission was denied. Enable it in your browser settings and refresh.',
    pushDisabled: 'Push notifications are temporarily disabled by the administrator.',
    pushEnableFailed: 'Failed to enable notifications',
    pushEnabled: 'Notifications enabled',
    pushDisabledLocally: 'Notifications disabled',
  },
  pushPrompt: {
    text: 'We’ll remind you to renew and help if you can’t connect',
    enable: 'Turn on',
    later: 'Not now',
    enabled: 'Notifications are on',
    blocked: 'Your browser has blocked notifications for this site. You can allow them in the browser settings.',
    notEnabled:
      'Notifications are off. You can turn them on later in Settings → Notifications → Notification settings.',
    failed:
      'Couldn’t turn on notifications. Try again later in Settings → Notifications → Notification settings.',
  },
  faq: {
    q1: 'How to connect VPN?',
    a1: 'Purchase a subscription, tap "Connect" on the subscription card, and copy the link. Paste it into your VPN client (V2RayNG, Hiddify, Streisand, etc.).',
    q2: 'How many devices can I connect?',
    a2: 'The number of devices depends on your plan. Check the limit on your subscription card.',
    q3: 'How to renew my subscription?',
    a3: 'Tap "Renew" under the subscription card, or buy a new plan — it will automatically extend your current subscription.',
    q4: 'What if VPN is not working?',
    a4: 'Try regenerating the connection link. If that doesn\'t help, create a ticket in the "Support" section.',
    loadFailedTitle: 'Help is temporarily unavailable',
    loadFailedBody: 'We could not load the latest FAQ from the service. Showing the built-in answers for now.',
    emptyTitle: 'Help is empty for now',
    emptyBody: 'The administrator has not added any active answers yet.',
    mediaGroupLabel: 'Materials for the answer “{{question}}”',
    mediaImageAlt: 'Illustration for the answer “{{question}}”',
    mediaVideoLabel: 'Video for the answer “{{question}}”',
    mediaLoadFailed: 'The media file could not be loaded.',
    mediaUnsupported: 'This file format cannot be previewed here.',
    mediaOpen: 'Open file',
  },
  statusBadge: {
    ACTIVE: 'Active',
    LIMITED: 'Limited',
    EXPIRED: 'Expired',
    DISABLED: 'Disabled',
    DELETED: 'Deleted',
    TRIAL: 'Trial',
  },
  intro: {
    skip: 'Skip',
    next: 'Next',
    start: 'Start',
    step1: {
      // {{brand}} is the operator's brand name. Never "Rezeis": that is the
      // panel's name, not the service the customer bought.
      title: 'Welcome to {{brand}}',
      description: 'A fast and reliable VPN for your safety online.',
    },
    step2: {
      title: 'Instant connection',
      description: 'Choose a plan, pay, and get your connection link in seconds.',
    },
    step3: {
      title: 'Referral program',
      description: 'Invite friends and earn bonus days or points for each of them.',
    },
    step4: {
      title: 'Promo codes & offers',
      description: 'Activate promo codes to get discounts and bonuses.',
    },
  },
  quests: {
    iconAria: 'Quests',
    title: 'Quests',
    subtitle: 'Complete quests and earn rewards.',
    pointsBalance: 'Points: {{count}}',
    exchangeLink: 'Exchange',
    claim: 'Claim',
    claiming: 'Claiming…',
    claimed: 'Reward received: {{reward}}',
    claimedPromo: 'Promo code: {{code}}',
    claimFailed: 'Failed to claim the reward',
    progress: '{{current}} / {{required}}',
    actions: {
      link: 'Link',
      invite: 'Invite',
      install: 'Install',
      openBot: 'Open bot',
    },
    reward: {
      points_one: '+{{count}} point',
      points_few: '+{{count}} points',
      points_many: '+{{count}} points',
      points_other: '+{{count}} points',
      days_one: '+{{count}} day',
      days_few: '+{{count}} days',
      days_many: '+{{count}} days',
      days_other: '+{{count}} days',
      discount: '+{{count}}% off',
      traffic: '+{{count}} GB',
      promocode: 'Promo code',
    },
    partner: {
      open: 'Open task',
      openVisit: 'Open',
      codePlaceholder: 'Activation code',
      submitCode: 'Verify',
      submitting: 'Verifying…',
      codeInvalid: 'Invalid code',
      confirmVisit: 'Confirm',
      waitSeconds: 'Confirm in {{count}}s',
      pendingPostback: 'Reward appears after the partner confirms',
      verified: 'Task verified',
      failed: 'Verification failed',
      newTab: '(opens in a new tab)',
    },
  },
  events: {
    title: 'Events',
    subtitle: 'The wheel is always on; contests run for a while',
    empty: 'Nothing is on right now. Come back later.',
    running: 'Running now',
    finished: 'Finished',
    unnamed: 'Contest',
    until: 'Until {{time}}',
    entries_one: '{{count}} entrant',
    entries_few: '{{count}} entrants',
    entries_many: '{{count}} entrants',
    entries_other: '{{count}} entrants',
    place: 'Place {{place}}:',
    enter: 'Enter',
    entered: 'You are in',
    youAreIn: 'You are in. Results after it ends.',
    enterFailed: 'Could not enter',
    wheel: {
      title: 'Wheel of fortune',
      canSpin: 'You have a spin — go',
      later: 'No spins right now',
    },
    refusal: {
      NOT_OPEN: 'Entries are closed.',
      NOT_STARTED: 'The contest has not started yet.',
      ENDED: 'Entries have closed.',
      NOT_ELIGIBLE: 'This contest is not for your plan.',
      FULL: 'No places left.',
    },
    result: {
      notThisTime: 'Not this time.',
      won: 'Place {{place}} — {{prize}}',
      pending: 'An operator will write to you in support to hand it over.',
      openChat: 'Open the conversation',
      refused: 'The prize was refused — details are in the conversation.',
    },
  },
  wheel: {
    title: 'Wheel of fortune',
    subtitle: 'Spin and take your prize',
    disabled: 'The wheel is not available right now.',
    spins_one: '{{count}} spin',
    spins_few: '{{count}} spins',
    spins_many: '{{count}} spins',
    spins_other: '{{count}} spins',
    points_one: '{{count}} point',
    points_few: '{{count}} points',
    points_many: '{{count}} points',
    points_other: '{{count}} points',
    spin: 'Spin',
    spinFree: 'Spin for free',
    noSpins: 'You have no spins left.',
    freeSpinAt: 'A free spin will be available {{time}}',
    buyOne: 'Buy a spin for {{price}} points',
    bought: 'Spin added',
    buyFailed: 'Could not buy a spin',
    spinFailed: 'Could not spin',
    historyLink: 'Spin history',
    prizeFallback: 'Prize',
    refusal: {
      WHEEL_DISABLED: 'The wheel is not available right now.',
      WHEEL_UNAVAILABLE: 'There is nothing to play for right now. Come back later.',
      NO_SPINS: 'You have no spins left.',
      USER_NOT_FOUND: 'We could not find your account.',
    },
    result: {
      won: 'You won!',
      keptInHistory: 'The code is kept in your spin history — you cannot lose it.',
      nothing: 'No luck this time',
      nothingHint: 'Try again when you have another spin.',
      manual: 'A big prize!',
      manualHint: 'An operator will write to you in support to hand it over.',
      close: 'Got it',
    },
    prize: {
      points_one: '{{count}} point',
      points_few: '{{count}} points',
      points_many: '{{count}} points',
      points_other: '{{count}} points',
      spins_one: '{{count}} spin',
      spins_few: '{{count}} spins',
      spins_many: '{{count}} spins',
      spins_other: '{{count}} spins',
      days_one: '{{count}} subscription day',
      days_few: '{{count}} subscription days',
      days_many: '{{count}} subscription days',
      days_other: '{{count}} subscription days',
      traffic: '{{count}} GB of traffic',
      discount: 'A permanent {{percent}}% discount',
      promocode: 'A promo code',
      key: 'A key',
    },
    history: {
      title: 'Spin history',
      empty: 'You have not spun yet.',
      pending: 'Waiting for an operator to hand it over.',
      openChat: 'Open the conversation',
      refused: 'The prize was refused — details are in the conversation.',
    },
  },
  pointsExchange: {
    title: 'Exchange points',
    unavailable: 'Points exchange is temporarily unavailable.',
    yourPoints: 'Your points',
    chooseType: 'Choose exchange type',
    rate: '{{cost}} points = 1 {{unit}}',
    unavailableShort: 'Unavailable',
    pointsAmount: 'Number of points',
    min: 'Min: {{value}}',
    max: 'Max: {{value}}',
    youReceive: 'You will receive',
    exchanging: 'Exchanging...',
    exchange: 'Exchange',
    success: 'Points exchanged!',
    syncPending: 'Reward applied. VPN profile synchronization is in progress.',
    error: 'Failed to exchange points',
    subscription: 'Subscription',
    noActiveSubscriptions: 'An active subscription is required for this exchange. Activate or purchase one first.',
    done: 'Done',
    giftCodeTitle: 'Promo code created',
    giftCodeHint: 'Share this promo code with a friend — it activates a subscription.',
    giftFixedHint: 'Fixed cost: {{cost}} points = 1 promo code.',
    history: 'History',
    types: {
      SUBSCRIPTION_DAYS: { label: 'Subscription days', unit: 'days' },
      GIFT_SUBSCRIPTION: { label: 'Gift subscription', unit: 'promo code' },
      DISCOUNT: { label: 'Discount', unit: '%' },
      TRAFFIC: { label: 'Traffic', unit: 'GB' },
    },
  },
  pointsHistory: {
    title: 'Points history',
    empty: 'Nothing has been earned yet',
    loadError: 'Failed to load the history',
    retry: 'Retry',
    showMore: 'Show more',
    balanceAfter_one: 'balance {{count}} point',
    // `_few` / `_many` exist only because `RuDict` is the contract and Russian
    // needs them; English resolves `_other` for both.
    balanceAfter_few: 'balance {{count}} points',
    balanceAfter_many: 'balance {{count}} points',
    balanceAfter_other: 'balance {{count}} points',
    sources: {
      CASHBACK: 'Purchase cashback',
      CASHBACK_REVERSED: 'Payment refunded',
      REFERRAL_REWARD: 'Invited friend reward',
      REFERRAL_REWARD_REVOKED: 'Friend reward revoked',
      QUEST_REWARD: 'Quest',
      WHEEL_PRIZE: 'Wheel of fortune',
      CONTEST_PRIZE: 'Contest',
      EXCHANGE: 'Points exchange',
      MANUAL_ADJUSTMENT: 'Operator adjustment',
      ACCOUNT_MERGE: 'Accounts merged',
      IMPORT: 'Transferred',
      OPENING_BALANCE: 'Opening balance',
    },
    reasons: {
      COMPENSATION: 'Compensation',
      PROMOTION: 'Promotion',
      CORRECTION: 'Correction',
      VIOLATION: 'Rules violation',
      OTHER: 'Other',
    },
    exchange: {
      SUBSCRIPTION_DAYS: 'subscription days',
      GIFT_SUBSCRIPTION: 'gift subscription',
      DISCOUNT: 'discount',
      TRAFFIC: 'traffic',
    },
    details: {
      cashback: '{{what}} · paid {{amount}}',
      days_one: '{{count}} day',
      days_few: '{{count}} days',
      days_many: '{{count}} days',
      days_other: '{{count}} days',
      reversal: '{{applied}} of {{requested}} taken back',
      shortfall_one: '{{count}} point short',
      shortfall_few: '{{count}} points short',
      shortfall_many: '{{count}} points short',
      shortfall_other: '{{count}} points short',
      quest: 'quest: {{title}}',
      exchange: 'exchange: {{type}}',
    },
  },
  openInBrowser: {
    title: 'Your account in the browser',
    body: 'Tap the button and your account opens in your browser, already signed in. Nothing to type.',
    openedBody: 'Your account is open in the browser. If nothing happened, tap again.',
    open: 'Open in browser',
    openAgain: 'Open again',
    stay: 'Stay in Telegram',
    failed: 'Could not prepare the sign-in. Try again, or stay in Telegram.',
    retry: 'Try again',
    mismatchBody:
      'The cabinet here is signed in as another Telegram account than the one that tapped “Cabinet”. Close this screen and tap “Cabinet” in the bot again — the cabinet of the account you tapped from will open.',
    relaunchBody:
      'Could not confirm which Telegram account opened this screen. Close it and tap “Cabinet” in the bot again.',
  },
  launchSwitch: {
    signingIn: 'Signing in…',
    signingInAs: 'Signing in as {{name}}…',
    failed: 'Could not sign in as the Telegram account that opened the app. Try again, or close the app and open it from the bot again.',
    retry: 'Try again',
  },
  authOpen: {
    signingIn: 'Signing you in…',
    hopTitle: 'Opening your browser',
    hopBody: 'If your browser did not open by itself, tap the button below.',
    hopButton: 'Open in browser',
    stayHere: 'Continue here',
    expiredTitle: 'This sign-in link has expired',
    expiredBody: 'Go back to the bot and tap «Кабинет» again, or sign in with your login and password.',
    signIn: 'Sign in',
  },
  support: {
    title: 'Support',
    newTicket: 'New ticket',
    contactTelegram: 'Contact support on Telegram',
    contactPrefill: 'Hello! I need help.',
    /** Full-screen AI assistant (header + entry points). */
    aiTitle: 'Quick help',
    quickHelp: 'Quick help',
    quickHelpAria: 'Open the AI assistant',
    aiSuggestion:
      'Try the AI assistant first — it answers about 90% of common questions (plans, VPN setup, apps) without a ticket.',
    tryAi: 'Ask AI',
    aiEmptyTitle: 'How can I help?',
    aiEmptyHint:
      'Ask about plans, VPN setup, apps, or common issues — I give guidance only. Personal actions (refunds, plan changes, access) need a human operator.',
    aiPlaceholder: 'Type your question…',
    aiError: 'Could not get a response. Try again or create a ticket.',
    aiInputLabel: 'Message to the AI assistant',
    aiSend: 'Send',
    aiThinking: 'AI is typing…',
    emptyTitle: 'You have no tickets',
    emptyHint: 'Create a ticket if you need help',
    emptyHintWithAi: 'You can ask AI first — many questions are solved without a ticket',
    statusOpen: 'Open',
    statusWaitingShort: 'Reply',
    statusClosed: 'Closed',
    chatStatusWaiting: 'Awaiting reply',
    chatAuthorSupport: 'Support',
    chatAuthorSystem: 'System',
    sendError: 'Failed to send',
    messagePlaceholder: 'Message...',
    attachAria: 'Attach a file',
    attachTooLarge: 'The file is too large',
    attachUnsupported: 'That file type is not accepted. PNG, JPEG, WEBP and PDF work.',
    attachFailed: 'Could not send the file',
    attachmentRemoved: '— file removed',
    ticketCreated: 'Ticket created',
    createError: 'Failed to create ticket',
    newTicketTitle: 'New ticket',
    subjectLabel: 'Subject',
    subjectPlaceholder: 'What is it about?',
    messageLabel: 'Message',
    messagePlaceholderLong: 'Describe the issue...',
    sending: 'Sending...',
    send: 'Send',
  },
};
